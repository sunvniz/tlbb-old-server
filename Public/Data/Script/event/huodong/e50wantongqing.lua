	...
	if (nowYear == 2008) then
		if(nowMonth==0) then
			if(nowDate>=16 and nowDate<=27) then
				AddNumText(sceneId, x808074_g_scriptId, "Tham gia #GHoÕt ðµng phän h°i m×ng tu±i", 1, 1 )
				AddNumText(sceneId, x808074_g_scriptId, "Gi¾i Thi®u #GhoÕt ðµng phän h°i m×ng tu±i", 11, 3 )
			end
		end
	end
...
function x808074_OnDefaultEvent( sceneId, selfId, targetId )
	...
	elseif TextNum == 1 then	
		...
		AddNumText(sceneId, x808074_g_scriptId, "Ta mu¯n tham gia", 8, 5 )
		AddNumText(sceneId, x808074_g_scriptId, "TÕi hÕ chï ði qua", 8, 6 )
		EndEvent( sceneId )
