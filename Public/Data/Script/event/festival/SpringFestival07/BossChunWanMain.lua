...
x050030_g_BOSSTbl = {
	...
	--ËïÃÀÃÀ....
	["SunMeiMei"] 		= { dateId = 3891, createMsg = "ÎÞ" },
	...
	["YingYing"] 			= { dateId = 3928, createMsg = "ÎÞ" },
	["ZhuZhu"]				= { dateId = 3926, createMsg = "ÎÞ" },
	["FeiTianMao"]		= { dateId = 3916, createMsg = "#{SpringFestiva2007_BOSS_13}" },
	["GouGou"]				= { dateId = 3927, createMsg = "ÎÞ" },
	["HuHu"]					= { dateId = 3929, createMsg = "ÎÞ" },
...
--BOSS×î´óÉú´æÊ±¼ä....(ÓÃÓÚ·ÀÖ¹ÍüÁËÉ¾³ý´ËBOSS)
...
x050030_g_IDXFireCount = 0 --µ±Ç°ÒÑ¾­È¼·ÅtoÕ ðµ ÑÌ»¨toÕ ðµ ÊýÁ¿....

x050030_g_IsPlaying = 0 --µ±Ç°Ðúng·ñÕýTÕi ²¥·Å½ÚÄ¿....
...
function x050030_PlayChapter( sceneId, chapterId )
	...
	if chapterId <= x050030_g_ChapterCount then
		...
		--Ê×²¥ºÍÖØ²¥Ê±²Å»áTÕi ½ÚÄ¿¿ªÊ¼toÕ ðµ Ê±ºò·¢¹«¸æ....
		if -1 ~= x050030_CheckActivityTime() then
...
function x050030_CreateBossActor( sceneId, strName, posX, posY, notify )
	...
	--Ê×²¥ºÍÖØ²¥Ê±²Å»áTÕi ´´½¨BOSStoÕ ðµ Ê±ºò·¢¹«¸æ....
	local notify2 = 1
	if -1 == x050030_CheckActivityTime() then
		notify2 = 0
	end
	...
	if notify and notify == 1 and notify2 == 1 then
		local msg = "#P[´ºÍíÖ÷³ÖÈË-ËïÃÀÃÀ]: #W"..bossData.createMsg
