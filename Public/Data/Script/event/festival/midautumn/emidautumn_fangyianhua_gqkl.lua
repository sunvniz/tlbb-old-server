...
x050019_g_Custom	= { {id="Ðã phóng Qu¯c Khánh Vui Vë",num=1} }
...
--»î¶¯Ê±¼ä
x050019_g_StartDayTime = 8257   --»î¶¯¿ªÊ¼Ê±¼ä 2008-9-14,°üº¬µ±ÈÕ
x050019_g_EndDayTime = 8282   --»î¶¯½áÊøÊ±¼ä 2008-10-09,°üº¬µ±ÈÕ
...
function x050019_OnAccept( sceneId, selfId, targetId )
	...
	Msg2Player(sceneId, selfId, "#YNh§n nhi®m vø: "..x050019_g_MissionName..".", MSG2PLAYER_PARA);
...
function x050019_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	if x050019_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		Msg2Player( sceneId, selfId,"#Y Hoàn thành nhi®m vø: "..x050019_g_MissionName..".",MSG2PLAYER_PARA )
...
function x050019_OnItemUsed( sceneId, selfId, itemdataId )
	...
	if xPos and zPos and sceneId
		...
		if misParam0 and misParam0 ~= 1 then
			...
			x050019_NotifyTips(sceneId, selfId, "Nhi®m vø ðã hoàn thành");
		end
	end
end
