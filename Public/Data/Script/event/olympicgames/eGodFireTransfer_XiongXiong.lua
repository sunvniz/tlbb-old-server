...
--ĞÂÊ¥»ğ´«µİ»î¶¯Ö®ĞÜĞÜtoÕ ğµ Ê¥»ğÊÂ¼ş½Å±¾
...
x808099_g_AccomplishNPC_Name="Ğao Nghiêu"
...
--ÈÎÎñĞúng·ñÒÑ¾­Íê³É
x808099_g_IsMissionOkFail	= 0		--ÈÎÎñ²ÎÊıtoÕ ğµ µÚ0Î»

--ÈÎÎñÎÄ±¾ÃèÊö
x808099_g_MissionName			= "ĞÜĞÜtoÕ ğµ Ê¥»ğ"
--ÈÎÎñÃèÊö
...
x808099_g_Custom	= { {id="ÒÑ½«»ğ¾æ½»¸øNhî HäitoÕ ğµ Ğao Nghiêu",num=1} }
--MisDescEnd

--ÈÎÎñĞúng·ñÍê³É
x808099_g_Mission_IsComplete = 0		--ÈÎÎñ²ÎÊıtoÕ ğµ µÚ0Î»
--Ê¥»ğ½»¸øĞao NghiêutoÕ ğµ ±ê¼Ç
x808099_g_RecordIdx 				 = 1		--ÈÎÎñ²ÎÊıtoÕ ğµ µÚ1Î»
--ÈÎÎñ½Å±¾ºÅ¼ÇÂ¼
x808099_g_MissScriptID_Idx	 = 2		--ÈÎÎñ²ÎÊıtoÕ ğµ µÚ2Î»
--ÈÎÎñ·¢²¼NPC±ê¼Ç
x808099_g_AcceptNPC_Idx			 = 3		--ÈÎÎñ²ÎÊıtoÕ ğµ µÚ3Î» 25.Nhî HäiNPC


x808099_g_AcceptMission_IDX		= 965	--ĞÜĞÜtoÕ ğµ Ê¥»ğ½ÓÊÕÈÎÎñË÷Òı
x808099_g_CompleteMission_IDX	= 966	--ĞÜĞÜtoÕ ğµ Ê¥»ğÌá½»ÈÎÎñË÷Òı
x808099_g_MissionInfo_IDX			= 967	--ÈÎÎñË ği¬m÷Ë÷Òı

--ËùÓµÓĞtoÕ ğµ ÊÂ¼şIDÁĞ±í
x808099_g_EventList	= {}

x808099_g_Impact_Accept_Mission 	 = 47		-- Tiªp thøÈÎÎñÊ±toÕ ğµ ÌØĞ§ID
x808099_g_Impact_Transport_Mission = 113 	-- ÔËÊä×´Ì¬ÌØĞ§
x808099_g_Impact_GodOfFireMan_Mission  = 5942 -- Ê¥»ğ´«µİÄĞ±äÉíÌØĞ§
x808099_g_Impact_GodOfFireGirl_Mission = 5943 -- Ê¥»ğ´«µİÅ®±äÉíÌØĞ§
x808099_g_Impact_DelGodOfFire_Mission  = 5944 -- É¾³ıÊ¥»ğÌØĞ§toÕ ğµ ÌØĞ§
x808099_g_PlayerSlow_LVL					 = 10		-- Tiªp thøÈÎÎñtoÕ ğµ ×îµÍµÈc¤p

x808099_g_Mission_StepNum					 = 3		-- ±¾ÈÎÎñTÕi ÈÎÎñÁ´toÕ ğµ µÚ¼¸²½

x808099_g_ItemId = { id=40004447,num=1 }	-- ÈÎÎñÎïÆ·
...
function x808099_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x808099_g_AcceptMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x808099_g_Name then
			x808099_NotifyTip( sceneId, selfId, "Nh§n nhi®m vø th¤t bÕi" )					
			return 0
		end
		-- ÈÎÎñĞúng·ñÒÑÂú
	...
	elseif key == x808099_g_CompleteMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x808099_g_AccomplishNPC_Name then
			x808099_NotifyTip( sceneId, selfId, "Trä nhi®m vø th¤t bÕi" )					
			return 0
		end
		-- Èç¹ûÒÑ¾­½ÓÁËÈÎÎñĞÜĞÜtoÕ ğµ Ê¥»ğ
	...
	if LuaFnGetName( sceneId, targetId ) == x808099_g_Name
		 ...
		 if IsHaveMission( sceneId, selfId, x808099_g_MissionId ) <= 0 then
				...
				if nStepNum == x808099_g_Mission_StepNum then
					AddNumText( sceneId, x808099_g_ScriptId, "Ta mu¯n tham gia: ĞÜĞÜtoÕ ğµ Ê¥»ğ", 7, x808099_g_AcceptMission_IDX )
				end
		 end
		-- AddNumText( sceneId, x808099_g_ScriptId, "ĞÜĞÜtoÕ ğµ Ê¥»ğ½éÉÜ", 11, x808099_g_MissionInfo_IDX )
	elseif LuaFnGetName( sceneId, targetId ) == x808099_g_AccomplishNPC_Name
		...
		if IsHaveMission( sceneId, selfId, x808099_g_MissionId ) > 0 then
			...
			if GetMissionParam(sceneId, selfId, misIndex, x808099_g_Mission_IsComplete) <= 0 then
				...
				AddNumText( sceneId, x808099_g_ScriptId, "ÎÒÒÑ¾­Íê³É: ĞÜĞÜtoÕ ğµ Ê¥»ğ", 7, x808099_g_CompleteMission_IDX )
			end
		end
	else
		return 0
...
function x808099_OnAbandon( sceneId, selfId )
  ...
  if HaveItem(sceneId, selfId, x808099_g_ItemId.id) > 0 then
  	if LuaFnGetAvailableItemCount(sceneId, selfId, x808099_g_ItemId.id) >= x808099_g_ItemId.num then
	  	...
	  	x808099_NotifyTip( sceneId, selfId, "V§t ph¦m hi®n tÕi không th¬ dùng ho£c ğã b¸ khoá!" )			
			return 0
  	end
  end
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x808099_OnKillObject( sceneId, selfId, objdataId ,objId)--²ÎÊıÒâË¼: ³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎï
end
