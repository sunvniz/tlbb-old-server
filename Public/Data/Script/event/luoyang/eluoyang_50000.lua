...
x250000_g_MissionTarget="    Hoàn thành nhi®m vø Sa Châu Lãnh."		--Møc tiêu nhi®m vø
x250000_g_ContinueInfo="Nhi®m vø ğã làm xong chßa ?"		--Î´Hoàn t¤t nhi®m vøtoÕ ğµ npc¶Ô»°
x250000_g_MissionComplete="Th§t là cám ½n các hÕ!"					--Hoàn t¤t nhi®m vønpcËµtoÕ ğµ »°
...
--ÈÎÎñÈë¿Úº¯Êı
--**********************************
function x250000_OnDefaultEvent( sceneId, selfId, targetId )	-- ği¬m»÷¸ÃÈÎÎñºóÖ´ĞĞ´Ë½Å±¾
...
function x250000_OnEnumerate( sceneId, selfId, targetId )
 		...
 		if GetName(sceneId,targetId) == x250000_g_Name then		--Èç¹ûĞúng·¢ÈÎÎñtoÕ ğµ npc		  
...
function x250000_OnMissionCheck( sceneId, selfId, npcid, scriptId, index1, index2, index3, indexpet )
  --PrintNum( indexpet )           
  --indexpet ´ËË÷ÒıÎª"¾ø¶ÔË÷ÒıÖµ",¼´TÕi 0-5 Õâ6cáiÀ¸Î»ÖĞtoÕ ğµ ĞòºÅ.
    ...
    local PetLevel = LuaFnGetPet_Level( sceneId, selfId, ValidIndex )		    -- ği¬mµ½ÕäÊŞc¤p±ğ				    
		
		local DataID = LuaFnGetPet_DataID( sceneId, selfId, ValidIndex )       -- ği¬mµ½ÕäÊŞID
    ...
    --PrintNum( PetLevel )				
		...
		if ret1 > 0 then     --³É¹¦É¾³ıÕäÊŞ		    		    
		    --PrintStr( "É¾³ı³É¹¦À­" )
		    --É¾³ıÕäÊŞºó,½«¸ù¾İÕäÊŞµÈc¤p¼ÆËãÍæ¼ÒËù ği¬m½ğÇ®ÊıÄ¿
