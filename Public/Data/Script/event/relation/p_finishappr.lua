...
function x806007_OnAccept( sceneId, selfId, targetId, tId )
	...
	if lv_t >= 40 and lv_t <= 45 and fp_st >= 300 and fp_ts >=300 then
		...
		--Ê¦µÂ ði¬mÏà¹Ø
		--¸øÊ¦¸µÔö¼ÓÊ¦µÂ(X=0-1000000) N=M£«MIN(X=500, Ê¦¸¸¶ÔÍ½µÜtoÕ ðµ ÓÑºÃ¶È*T)
		--local	_M	= 10		--»ùÊý
		--local	_T	= 5			--¼ÓÈ¨Ö¸Êý
		--local	MoralPoint	--Ê¦µÂ ði¬mÊý
		...
		--Msg2Player( sceneId, selfId, " ði¬mµ½Ê¦µÂ ði¬mÊý"..MoralPoint..".", MSG2PLAYER_PARA )
		...
		--	Msg2Player( sceneId, selfId, " ðÕt ðßþc³ÆºÅ: "..title..".", MSG2PLAYER_PARA )
		--end
	...
	--(2)Èç¹ûÍ½µÜµÈc¤p´óÓÚ45,»òÕßË«·½ÓÑºÃ¶ÈÐ¡ÓÚ300
	else
