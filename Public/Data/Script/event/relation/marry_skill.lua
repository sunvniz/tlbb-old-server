...
x806016_g_skillList_TQLZ = {
	...
	{id=250, name="Ð°ng khí liên chi (1 c¤p)", firendPt=1000, lvM=35, lvF=20, exp=24739, money=10000},
	{id=251, name="Ð°ng khí liên chi (2 c¤p)", firendPt=3000, lvM=45, lvF=30, exp=53745, money=40000},
	{id=252, name="Ð°ng khí liên chi (3 c¤p)", firendPt=5000, lvM=55, lvF=40, exp=116762, money=100000},
	{id=253, name="Ð°ng khí liên chi (4 c¤p)", firendPt=7000, lvM=65, lvF=50, exp=253665, money=200000},
	{id=254, name="Ð°ng khí liên chi (5 c¤p)", firendPt=9999, lvM=75, lvF=60, exp=551086, money=500000},
...
x806016_g_skillList_TCDJ = {
	...
	{id=255, name="Thiên trß¶ng ð¸a cØu (1 c¤p)", firendPt=1000, lvM=35, lvF=20, exp=24739, money=10000},
	{id=256, name="Thiên trß¶ng ð¸a cØu (2 c¤p)", firendPt=3000, lvM=45, lvF=30, exp=53745, money=40000},
	{id=257, name="Thiên trß¶ng ð¸a cØu (3 c¤p)", firendPt=5000, lvM=55, lvF=40, exp=116762, money=100000},
	{id=258, name="Thiên trß¶ng ð¸a cØu (4 c¤p)", firendPt=7000, lvM=65, lvF=50, exp=253665, money=200000},
	{id=259, name="Thiên trß¶ng ð¸a cØu (5 c¤p)", firendPt=9999, lvM=75, lvF=60, exp=551086, money=500000},

};

--Hình änh b¤t li ¼¼ÄÜ±í....
x806016_g_skillList_XYBL = {

	{id=269, name="Hình änh b¤t li (1 c¤p)", firendPt=1000, lvM=35, lvF=20, exp=24739, money=10000},
	{id=270, name="Hình änh b¤t li (2 c¤p)", firendPt=3000, lvM=45, lvF=30, exp=53745, money=40000},
	{id=271, name="Hình änh b¤t li (3 c¤p)", firendPt=5000, lvM=55, lvF=40, exp=116762, money=100000},
	{id=272, name="Hình änh b¤t li (4 c¤p)", firendPt=7000, lvM=65, lvF=50, exp=253665, money=200000},
	{id=273, name="Hình änh b¤t li (5 c¤p)", firendPt=9999, lvM=75, lvF=60, exp=551086, money=500000},

};

--Hình änh b¤t li 1c¤p¶ÔÓ¦toÕ ðµ ¼¼ÄÜID
x806016_g_xybl_SkillID = 269
--H÷c t§p Hình änh b¤t li 1c¤pC¥n toÕ ðµ ÎïÆ·ID
x806016_g_xybl_ItemID = 30308059

--C¥n ÊÀ½ç¹«¸ætoÕ ðµ ¼¼ÄÜ±àºÅ±í
...
function x806016_OnDefaultEvent(sceneId, selfId, targetId)
	...
	if selectEventId == 0 then
		BeginEvent(sceneId);
			...
			AddNumText(sceneId, x806016_g_ScriptId, "H÷c t§p Hình änh b¤t li ", 12, 14);
		EndEvent(sceneId);
	...
	elseif selectEventId == 11 then
		...
		if ret > 0 then
			...
			BeginEvent(sceneId);
				...
				if nextLevel > 0 then
					AddNumText(sceneId, x806016_g_ScriptId, "KÛ nång thång c¤p", 12, 21);
				else
					AddNumText(sceneId, x806016_g_ScriptId, "L§p tÑc h÷c t§p", 12, 21);
				end
			EndEvent(sceneId);
			DispatchEventList(sceneId, selfId, targetId);
		end
	...
	elseif selectEventId == 12 then
		...
		if ret > 0 then
			...
			BeginEvent(sceneId);
				...
				if nextLevel > 0 then
					AddNumText(sceneId, x806016_g_ScriptId, "KÛ nång thång c¤p", 12, 22);
				else
					AddNumText(sceneId, x806016_g_ScriptId, "L§p tÑc h÷c t§p", 12, 22);
				end
			EndEvent(sceneId);
			DispatchEventList(sceneId, selfId, targetId);
		end
	...
	elseif selectEventId == 13 then
		...
		if ret > 0 then
			...
			BeginEvent(sceneId);
				...
				AddText(sceneId, "  Ð°ng th¶i c¥n vþ ch°ng häo hæu ðµ ðÕt t¾i#G"..skill.firendPt.."#W, nhà trai c¤p b§c ðÕt t¾i "..skill.lvM.."c¤p, nhà gái c¤p b§c ðÕt t¾i "..skill.lvF.."c¤p.");
				if nextLevel > 0 then
					AddNumText(sceneId, x806016_g_ScriptId, "KÛ nång thång c¤p", 12, 23);
				else
					AddNumText(sceneId, x806016_g_ScriptId, "L§p tÑc h÷c t§p", 12, 23);
				end
			EndEvent(sceneId);
			DispatchEventList(sceneId, selfId, targetId);
		end
	...
	elseif selectEventId == 14 then
		...
		if ret > 0 then
			...
			BeginEvent(sceneId);
				AddText(sceneId, "  Hình änh b¤t li có th¬ trong nháy m¡t di chuy¬n ðªn v¸ trí vþ ch°ng, kÛ nång c¤p b§c càng cao, th¶i gian càng h°i phøc ít.");
				...
				AddText(sceneId, "  Ð°ng th¶i c¥n vþ ch°ng häo hæu ðµ ðÕt t¾i#G"..skill.firendPt.."#W, nhà trai c¤p b§c ðÕt t¾i "..skill.lvM.."c¤p, nhà gái c¤p b§c ðÕt t¾i "..skill.lvF.."c¤p.");
				if nextLevel > 0 then
					AddNumText(sceneId, x806016_g_ScriptId, "KÛ nång thång c¤p", 12, 24);
				else
					AddNumText(sceneId, x806016_g_ScriptId, "L§p tÑc h÷c t§p", 12, 24);
				end
			EndEvent(sceneId);
			DispatchEventList(sceneId, selfId, targetId);
		end
...
--**********************************
--¼ì²âÐúng·ñ¿ÉÒÔH÷c t§p Ä³Ïî·òÆÞ¼¼ÄÜ....
--**********************************
function x806016_CheckStudySkill( sceneId, selfId, targetId, SkillList )
	...
	skillLevel = 0;
	--¼ì²éÍæ¼ÒtoÕ ðµ ·òÆÞ¼¼ÄÜÒÑ¾­Ñ§µ½¼¸c¤p
...
--H÷c t§p Hình änh b¤t li 1c¤pÊ±toÕ ðµ ÌáÊ¾ÐÅÏ¢
function x806016_SendSkillMsg_XYBL(sceneId,selfId,skname)
	if skname then
		local str = format("#IQuý vþ ch°ng ð°ng tâm hi®p lñc, bài tr× muôn vàn khó khån, · nhà trai hao phí #Y#{_ITEM"..tostring(x806016_g_xybl_ItemID).."}#Ir¯t cøc h÷c xong kÛ nång phu thê#Y%s.",skname)
		BeginEvent(sceneId)
			AddText(sceneId, str);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
...
function x806016_SendSkillMsg(sceneId,selfId,exp,money,skname)
	if exp and money and skname then
		...
		local str = format("#IQuý vþ ch°ng ð°ng tâm hi®p lñc, bài tr× muôn vàn khó khån, · nhà trai hao phí  #Y%d kinh nghi®m cùng %s ti«n tài #Ir¯t cøc h÷c xong kÛ nång phu thê #Y%s.",exp,moneyt,skname)
		BeginEvent(sceneId)
			AddText(sceneId, str);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	...
	if maleId and femaleId and skill then
		for _, tempId in x806016_g_MaxMarrySkill_T do
			if tempId == skill.id then
					...
					local str = format("#W%s#I cùng #W%s#I vþ ch°ng ð°ng tâm, r¯t cøc kh± tçn cam lai, h÷c xong #cff66cc%s#I, không h± là s· hæu vþ ch°ng  mçu.",uname,oname,skill.name)
...
function x806016_LogCoupleAction(sceneId,tid1,tid2,SkillList,nextLevel)
	...
	if skill and logid and COUPLE_LOG_DETAIL[logid] then
		--CPL:±àºÅ,Ë ði¬m÷,GUID1,GUID2,¼¼ÄÜID,¼¼ÄÜÃû³Æ,Ê±¼ä
