...
function x713577_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if AbilityLevel < 1	then
		BeginEvent(sceneId)
			strText = "Ngß½i vçn chßa h÷c kÛ nång chª tÕo bùa"
	...
	if AbilityLevel >= x713577_g_nMaxLevel then
		BeginEvent(sceneId)
			strText = "Ta chï có th¬ dÕy ngß½i kÛ nång chª tÕo bùa t× c¤p 1-10, hãy t¾i bang phái ð¬ h÷c c¤p cao h½n"
			AddText(sceneId,strText)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	else
...
function x713577_OnEnumerate( sceneId, selfId, targetId )
		...
		if 1 then
			AddNumText(sceneId,x713577_g_ScriptId,"Thång c¤p kÛ nång chª tÕo bùa", 12, 1)
		end
		return
end
