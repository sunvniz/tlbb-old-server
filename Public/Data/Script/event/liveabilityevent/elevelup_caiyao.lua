...
--¼¼ÄÜÃû³Æ
x713568_g_AbilityName = " Hái dßşc "
...
function x713568_OnDefaultEvent( sceneId, selfId, targetId, nNum, npcScriptId, bid )
	...
	if AbilityLevel < 1	then
		BeginEvent(sceneId)
			strText = "Ngß½i vçn chßa th¬ h÷c"..x713568_g_AbilityName.." kÛ nång!"
	...
	if AbilityLevel >= x713568_g_MaxLevel then
		BeginEvent(sceneId)
			strText = "Ta chï có th¬ dÕy ngß½i t× c¤p 1-5"..x713568_g_AbilityName.." kÛ nång, xin t¾i bang hµi ğ¬ h÷c cao h½n!"..x713568_g_AbilityName.."."
			AddText(sceneId,strText)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	else
...
function x713568_OnEnumerate( sceneId, selfId, targetId, bid )
		if bid then
			...
			if ret > 0 then AddNumText(sceneId,x713568_g_ScriptId,"Thång c¤p"..x713568_g_AbilityName.." kÛ nång", 12, 1) end
			return
		end
		...
		if ret and ret == 1 and 1 then
			AddNumText(sceneId,x713568_g_ScriptId,"Thång c¤p"..x713568_g_AbilityName.." kÛ nång", 12, 1)
		end
		return
end
