...
--H÷c ½çÃæÒªËµtoÕ ðµ »°
x713506_g_MessageStudy = "Nªu các hÕ ðÕt t¾i c¤p %d và phäi tiêu t¯n #{_EXCHG%d} m¾i có th¬ h÷c ðßþc kÛ nång may m£c. Các hÕ có quyªt ð¸nh h÷c không?"
...
function x713506_OnDefaultEvent( sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId,bid )
	...
	--Íæ¼Ò·ìÈÒ kÛ nångtoÕ ðµ ÊìÁ·¶È
	ExpPoint = GetAbilityExp(sceneId, selfId, x713506_g_AbilityID)
	--ÈÎÎñÅÐ¶Ï

	--ÅÐ¶ÏÐúng·ñÒÑ¾­Ñ§»áÁË·ìÈÒ,Èç¹ûÑ§»áÁË,ÔòÌáÊ¾ÒÑ¾­Ñ§»áÁË
...
function x713506_StudyInCity(sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId,bid)
	if bid then
		if 0 == ButtomNum then
			...
			local studyMsg = format("Nªu các hÕ ðÕt t¾i c¤p %d, phäi tiêu t¯n #{_EXCHG%d} và %d ði¬m bang hµi s¨ có th¬ h÷c ðßþc"..x713506_g_AbilityName.." KÛ nång. Ngß½i quyªt ð¸nh h÷c cái gì?", lv, money, con)
