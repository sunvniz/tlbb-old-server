...
--½±Àø°²±£½ðÅÆ
--x210245_g_AnBaoJinPai = 30008046	--°²±£½ðÅÆ 
...
function x210245_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if dkey == 110 then
		x210245_MsgBox(sceneId, selfId, "#{RHD_80904_01}") --ÔÝÊ±¹Ø±ÕLînh Èó»êµ¤,38667
...
--      end
	...
	elseif dkey == 112 then
		...
		elseif IsMacBangding == 0 and IsIpBangding == 0 then
			...
			--if delret < 0 then	 --Èç¹ûÉ¾³ýth¤t bÕi,½«²»»á¸øÍæ¼Ò´óÀñ°ü
				--return
			--end
			...
			AuditAnBaoJinPaiZengPing(sceneId, selfId,x210245_g_AnBaoDaLiBao)		--¼ÇÂ¼°²±£´óÀñ°ütoÕ ðµ Lînh 
...
--**********************************
--Íæ¼ÒÉÏÏßÖ®ºótoÕ ðµ ÓÊ¼þÌáÊ¾,scene.luatoÕ ðµ x888888_OnScenePlayerLoginµ÷ÓÃµ½ÕâÀï
--**********************************
function x210245_SendMail( sceneId, selfId, nowtime )

	LuaFnSendSystemMail( sceneId, GetName(sceneId, selfId), "#{DHMB_08820_01}" )   -- ·¢ÓÊ¼þ
