-- ËÍĞÅÈÎÎñ(ËùÓĞÕâmµt ÀàĞÍ¶¼Ê¹ÓÃCái này LUA,ĞŞ¸ÄÇë×¢ÒâÍ¨ÓÃĞÔ)
...
-- °´ÕÕË³Ğò´æ´¢mµt Ğ©Éæ¼°µ½¿Í»§¶ËMøc tiêu nhi®m vøÏÔÊ¾toÕ ğµ ¶¯Ì¬ĞÅÏ¢
-- ¹æÔò: TÕi ÈÎºÎÇé¿öÏÂ,Ã¿mµt ÖÖ¿ÉÄÜËæ»útoÕ ğµ ÀàĞÍ×î¶àÖ»ÓĞmµt cáiËæ»úÏî
-- ¼´Í¬Ê±×î¶àÖ»ÄÜÓµÓĞ 1 cáiËæ»ú NPC(¹ÖÎï)¡¢1 cáiËæ»úÎïÆ·ºÍ 1 cáiËæ»úÕäÊŞ
-- Î»ÖÃ°´ÕÕÓÅÏÈË³Ğò´æ´¢,ÀıÈçÈç¹ûÓĞËæ»ú NPC,ÔòËæ»ú NPC Õ¼¾İµÚmµt cáiÈÎÎñÊı¾İ
-- ·ñÔòËæ»úÎïÆ·Õ¼¾İµÚmµt cáiÈÎÎñÊı¾İ,ÒÀ´ËÀàÍÆ
-- ¸ÃÖµ×îºÃ²»ÒªĞŞ¸Ä,¼´Ê¹ÒªĞŞ¸ÄÒ²ÒªºÍ¿Í»§¶ËtoÕ ğµ ½âÎö²¿·Ö(SetTableMissionInfo)Í¬Ê±ĞŞ¸Ä
...
--**********************************
-- »ñÈ¡¶¯Ì¬Møc tiêu nhi®m vø NPC toÕ ğµ ´æ´¢Î»ÖÃ
-- Èç¹ûĞúng¾²Ì¬Møc tiêu nhi®m vø NPC,ÔòTr· v« -1
--**********************************
function x006668_GetTNpcStoreLoc( sceneId, selfId, targetId, missionIndex )
	...
	if not isTargetDynamic or isTargetDynamic ~= 1 then					-- ¸Ã NPC Ğúng¾²Ì¬ NPC
		return -1
	end
...
--**********************************
-- »ñÈ¡¶¯Ì¬Møc tiêu nhi®m vø NPC toÕ ğµ  NPCID
-- Èç¹ûĞúng¾²Ì¬Møc tiêu nhi®m vø NPC,ÔòTr· v« -1
--**********************************
...
--**********************************
-- »ñÈ¡¶¯Ì¬ÈÎÎñÎïÆ·toÕ ğµ  ItemIndex
-- Èç¹ûĞúng¾²Ì¬ÈÎÎñÎïÆ·,ÔòTr· v« -1
--**********************************
...
--**********************************
-- ¿Û³ıÈÎÎñÎïÆ·,³É¹¦Tr· v« 1,th¤t bÕiTr· v« 0
--**********************************
function x006668_TakeOutMissionItem( sceneId, selfId, missionIndex )
	...
	if ItemCount > 0 then
		-- mi[5][1], mi[5][2], mi[5][3] ĞúngËæ»úÎïÆ·Ïî
		if mi[5][item] > 0 then
			-- mi[5][item] toÕ ğµ ×ª»»
...
-- ÈÎÎñÈë¿Úº¯Êı
--**********************************
function x006668_OnDefaultEvent( sceneId, selfId, targetId, missionIndex )		--  ği¬m»÷¸ÃÈÎÎñºóÖ´ĞĞ´Ë½Å±¾
	local missionId = TGetMissionIdByIndex( missionIndex )
	...
	if IsHaveMission( sceneId, selfId, missionId ) > 0 then
		...
		-- Èç¹ûMøc tiêu nhi®m vø NPC ºÍÈÎÎñÍê³É NPC ²»ĞúngÍ¬mµt cáiÈË
		if targetNPCName ~= completeNpcName then
			local bAchieveMission = 0

			-- Èç¹ûµ±Ç° NPC ĞúngMøc tiêu nhi®m vø NPC
			if not isTargetDynamic or isTargetDynamic ~= 1 then			-- ¸Ã NPC Ğúng¾²Ì¬ NPC
			...
			-- ÔòÈÎÎñÍê³É,²¢ÏÔÊ¾Møc tiêu nhi®m vø NPC ¶Ô°×
			if bAchieveMission == 1 then
				-- Èç¹ûC¥n ÈÎÎñÎïÆ·,²¢ÇÒÄÜ¹»É¾³ı,Ôò³É¹¦
		...
		elseif GetMissionParam( sceneId, selfId, misIndex, x006668_g_IsMissionOkFail ) ~= 1 then
			-- Èç¹ûMøc tiêu nhi®m vø NPC ºÍÈÎÎñÍê³É NPC ĞúngÍ¬mµt cáiÈË,²¢ÇÒCái này ¼mµt ï»¹Ã»ÓĞHoàn t¤t nhi®m vø
			if x006668_TakeOutMissionItem( sceneId, selfId, missionIndex ) == 1 then
				SetMissionByIndex( sceneId, selfId, misIndex, x006668_g_IsMissionOkFail, 1 )
			end
		end

		-- Èç¹ûµ±Ç° NPC ĞúngÈÎÎñÍê³É NPC,Ôò°´ÕÕÔ­ÓĞÁ÷³ÌÏÔÊ¾ÈÎÎñ¶Ô°×ÒÔ¼°Ìá½»ÈÎÎñ
...
--**********************************
-- Tiªp thø,Tr· v« 1 ±íÊ¾½ÓÈ¡³É¹¦,·ñÔò±íÊ¾th¤t bÕi
--**********************************
function x006668_OnAccept( sceneId, selfId, targetId, missionIndex )
	...
	if ItemCount > 0 then
		...
		-- mi[5][1], mi[5][2], mi[5][3] ĞúngËæ»úÎïÆ·Ïî
		if mi[5][item] > 0 then
			-- mi[5][item] toÕ ğµ ×ª»»TÕi ÕâÀïmµt ¶¨Òª×ö,ÒòÎªÒÔºóÒªÓÃµ½
			mi[5][item] = GetOneMissionItem( mi[5][item] )
	...
	if targetNpcStoreLocation ~= -1 then					-- ¸Ã NPC Ğúng¶¯Ì¬ NPC
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x006668_OnKillObject( sceneId, selfId, objdataId, objId, missionIndex )	-- ²ÎÊıÒâË¼: ³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
