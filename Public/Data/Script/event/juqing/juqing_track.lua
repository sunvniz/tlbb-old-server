...
function x200098_DispatchJuqintInfo(sceneId, selfId, targetId)
	...
	if nLevel < 20 then
		BeginEvent(sceneId)	
			AddText( sceneId, "  Hi®n gi¶ ðÆng c¤p cüa ngß½i chßa t¾i 20, không th¬ kh·i ðµng nhi®m vø tình tiªt, sau khi ðÕt t¾i c¤p 20, hÇng t¾i tìm ta" )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
		
	else
		-- 1,¼ì²âÍæ¼Ò×ö¹ýÄÄÐ©ÄÜ×ötoÕ ðµ ÈÎÎñ
		for i=2, 47   do
		...
		-- 2,¼ì²âÍæ¼Ò×ö¹ýÄÄÐ©ÄÜ×ötoÕ ðµ ÈÎÎñ
		for i=2, 47   do
		...
		-- 3,¼ì²âÍæ¼ÒÏÖTÕi ¿ÉÒÔ½ÓtoÕ ðµ ÈÎÎñ
		if HaveMis == 1  then
			CanMis = DoneMis + 1
		end
	...
	-- Íæ¼ÒÍê³ÉÁËËùÓÐµ±Ç°ÖÆ×÷Íê³ÉtoÕ ðµ ¾çÇéÈÎÎñ
	if IsMissionHaveDone(sceneId,selfId,45) > 0 then
		szText = "Gi¶ ðây ngß½i ðã hoàn t¤t nhi®m vø tình tiªt #GVÕn Kiªp C¯c, Yªn TØ ±, Tø Hi«n Trang, Thß½ng Mang S½n, Lôi C± S½n#W và #GNh¤t Ph¦m Ðß¶ng #W"
	end
...
--		AddText( sceneId, "ÄãÒÑ¾­Íê³ÉtoÕ ðµ ÈÎÎñÐúng =" .. x200098_g_Juqing_1[DoneMis].MisId )
--		AddText( sceneId, "ÄãÏÖTÕi ½Óµ½toÕ ðµ ÈÎÎñÐúng =" .. x200098_g_Juqing_1[HaveMis].MisId )
--		AddText( sceneId, "ÄãÏÖTÕi ¿ÉÒÔ½Óµ½toÕ ðµ ÈÎÎñÐúng =" .. x200098_g_Juqing_1[CanMis].MisId )
--	EndEvent(sceneId)
	DispatchEventList(sceneId,selfId,targetId)
