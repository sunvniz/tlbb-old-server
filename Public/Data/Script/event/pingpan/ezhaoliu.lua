...
--Ñ­»·ÈÎÎñtoÕ ğµ Êı¾İË÷Òı,ÀïÃæ´æ×ÅÒÑ×ötoÕ ğµ »·Êı MD_PINPAN_HUAN
...
x311005_g_MissionTarget="Nh¶ sñ giúp ğŞ cüa Lßu Ki®n Minh tiªn vào doanh ğ¸a cüa phän quân, giªt chªt 1 tên Phän Quân Thü Lînh, 13 tên Phän Quân Thü V®, 6 tên Phän Quân Môn V®."	--Møc tiêu nhi®m vø
x311005_g_ContinueInfo="Các hÕ phäi tiªp tøc c¯ g¡ng nhé!"	--Î´Hoàn t¤t nhi®m vøtoÕ ğµ npc¶Ô»°
x311005_g_MissionComplete="Cám ½n, chúng tôi cu¯i cùng cûng ğã dám ra khöi cØa r°i"	--Hoàn t¤t nhi®m vønpcËµ»°toÕ ğµ »°
...
--0ºÅ: ÒÑ¾­Íê³ÉtoÕ ğµ »·Êı,TÕi ½ÓÊÕÈÎÎñÊ±ºò¸³Öµ
--1ºÅ: µ±Ç°ÈÎÎñĞúng·ñÍê³É(0Î´Íê³É£»1Íê³É)
--2ºÅ: µ±Ç°¸±±¾ÈÎÎñtoÕ ğµ ³¡¾°ºÅ
--3ºÅ: ½Ó¸±±¾ÈÎÎñÊ±ºòtoÕ ğµ ¶ÓÎéºÅ
--4ºÅ: Î´ÓÃ
--5ºÅ: Î´ÓÃ
--6ºÅ: Î´ÓÃ
--7ºÅ: Î´ÓÃ

x311005_g_LimitMembers = 1 --¿ÉÒÔ½ø¸±±¾toÕ ğµ ×îĞ¡¶ÓÎéÈËÊı
x311005_g_TickTime=5 --»Øµ÷½Å±¾toÕ ğµ Ê±ÖÓÊ±¼ä(µ¥Î»:  giây/´Î)
x311005_g_LimitTotalHoldTime=120 --¸±±¾¿ÉÒÔ´æ»îtoÕ ğµ Ê±¼ä(µ¥Î»: ´ÎÊı)
x311005_g_CloseTick=6	--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±(µ¥Î»: ´ÎÊı)
x311005_g_NoUserTime=300 --¸±±¾ÖĞÃ»ÓĞÈËºó¿ÉÒÔ¼ÌĞø±£´ætoÕ ğµ Ê±¼ä(µ¥Î»:  giây)
...
function x311005_OnDefaultEvent( sceneId, selfId, targetId )
	if( IsHaveMission(sceneId,selfId,x311005_g_MissionId) > 0)  then	--Èç¹ûÍæ¼ÒÒÑ¾­½ÓÁËCái này ÈÎÎñ
		...
		if GetName(sceneId,targetId) == x311005_g_Name then		--ĞúngÄ¿±êNPC
			BeginEvent(sceneId)
		...
		elseif bDone==1 then	--ÈÎÎñÒÑ¾­Íê³É,²¢ÇÒTÕi ½»ÈÎÎñtoÕ ğµ NPCÉÏ ği¬m»÷toÕ ğµ ÊÂ¼ş
			BeginEvent(sceneId)
				AddText(sceneId,x311005_g_MissionName)
    ...
    elseif x311005_CheckAccept(sceneId,selfId) > 0 then		--Ã»ÓĞÈÎÎñ,Thöa mãnÈÎÎñ½ÓÊÕÌõ¼ş
...
function x311005_OnEnumerate( sceneId, selfId, targetId )
	...
	if IsHaveMission(sceneId,selfId,x311005_g_MissionId) > 0 then
		...
		elseif	GetName(sceneId,targetId) ~= x311005_g_Name	and	bDone==1 then --½»ÈÎÎñtoÕ ğµ NPC, ÈÎÎñÍê³É
			AddNumText(sceneId, x311005_g_ScriptId,x311005_g_MissionName);
		end
...
function x311005_OnAccept( sceneId, selfId )
	...
	if( IsHaveMission(sceneId,selfId,x311005_g_MissionId) > 0)  then	--ÒÑ¾­ÓĞÈÎÎñtoÕ ğµ 
		misIndex = GetMissionIndexByID(sceneId,selfId,x311005_g_MissionId)
		...
		if copysceneid<0 then --»¹Ã»ÓĞ½ø¹ı¸±±¾
			...
			if nearmembercount>0 then --ÓĞ¶ÓÎé,ÈËÊı×ã¹»,Îª¶Ó³¤, Tr· v«ÖµÎªÈËÊı
			--¶Ó³¤´´½¨¸±±¾³¡¾°
...
function x311005_MakeCopyScene( sceneId, selfId, nearmembercount )
	...
	LuaFnSetSceneLoad_Map(sceneId, "_pingpan_1.nav"); --µØÍ¼Ğúng±ØĞëÑ¡È¡toÕ ğµ ,¶øÇÒ±ØĞëTÕi Config/SceneInfo.iniÀïÅäÖÃºÃ
	...
	LuaFnSetCopySceneData_Param(sceneId, 0, 999);--ÉèÖÃ¸±±¾Êı¾İ,ÕâÀï½«0ºÅË÷ÒıtoÕ ğµ Êı¾İÉèÖÃÎª999,ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êı×Ö×Ô¶¨Òå)
	...
	LuaFnSetCopySceneData_Param(sceneId, 6, 0);--É±ËÀtoÕ ğµ ÅÑ¾ü1toÕ ğµ ÊıÁ¿
	LuaFnSetCopySceneData_Param(sceneId, 7, 0);--É±ËÀtoÕ ğµ ÅÑ¾ü2toÕ ğµ ÊıÁ¿
	LuaFnSetCopySceneData_Param(sceneId, 8, 0);--É±ËÀtoÕ ğµ ÅÑ¾ü3toÕ ğµ ÊıÁ¿
	
	if	mylevel<=40	 then
		LuaFnSetSceneLoad_Monster(sceneId, "_pingpan_1_monster.ini")
		LuaFnSetCopySceneData_Param(sceneId, 9,536);--É±ËÀtoÕ ğµ ÅÑ¾ü1toÕ ğµ ¹ÖÎïÀàĞÍ
		LuaFnSetCopySceneData_Param(sceneId,10,529);--É±ËÀtoÕ ğµ ÅÑ¾ü2toÕ ğµ ¹ÖÎïÀàĞÍ
		LuaFnSetCopySceneData_Param(sceneId,11,538);--É±ËÀtoÕ ğµ ÅÑ¾ü3toÕ ğµ ¹ÖÎïÀàĞÍ
	elseif	mylevel<=55	 then
		LuaFnSetSceneLoad_Monster(sceneId, "_pingpan_40_monster.ini")
		LuaFnSetCopySceneData_Param(sceneId, 9,1825);--É±ËÀtoÕ ğµ ÅÑ¾ü1toÕ ğµ ¹ÖÎïÀàĞÍ
		LuaFnSetCopySceneData_Param(sceneId,10,1835);--É±ËÀtoÕ ğµ ÅÑ¾ü2toÕ ğµ ¹ÖÎïÀàĞÍ
		LuaFnSetCopySceneData_Param(sceneId,11,539);--É±ËÀtoÕ ğµ ÅÑ¾ü3toÕ ğµ ¹ÖÎïÀàĞÍ
	elseif	mylevel<=70	 then
		LuaFnSetSceneLoad_Monster(sceneId, "_pingpan_55_monster.ini")
		LuaFnSetCopySceneData_Param(sceneId, 9,1845);--É±ËÀtoÕ ğµ ÅÑ¾ü1toÕ ğµ ¹ÖÎïÀàĞÍ
		LuaFnSetCopySceneData_Param(sceneId,10,1850);--É±ËÀtoÕ ğµ ÅÑ¾ü2toÕ ğµ ¹ÖÎïÀàĞÍ
		LuaFnSetCopySceneData_Param(sceneId,11,540);--É±ËÀtoÕ ğµ ÅÑ¾ü3toÕ ğµ ¹ÖÎïÀàĞÍ
	elseif	mylevel<=85	 then
		LuaFnSetSceneLoad_Monster(sceneId, "_pingpan_70_monster.ini")
		LuaFnSetCopySceneData_Param(sceneId, 9,517);--É±ËÀtoÕ ğµ ÅÑ¾ü1toÕ ğµ ¹ÖÎïÀàĞÍ
		LuaFnSetCopySceneData_Param(sceneId,10,518);--É±ËÀtoÕ ğµ ÅÑ¾ü2toÕ ğµ ¹ÖÎïÀàĞÍ
		LuaFnSetCopySceneData_Param(sceneId,11,541);--É±ËÀtoÕ ğµ ÅÑ¾ü3toÕ ğµ ¹ÖÎïÀàĞÍ
	else
		LuaFnSetSceneLoad_Monster(sceneId, "_pingpan_85_monster.ini")
		LuaFnSetCopySceneData_Param(sceneId, 9,1955);--É±ËÀtoÕ ğµ ÅÑ¾ü1toÕ ğµ ¹ÖÎïÀàĞÍ
		LuaFnSetCopySceneData_Param(sceneId,10,1950);--É±ËÀtoÕ ğµ ÅÑ¾ü2toÕ ğµ ¹ÖÎïÀàĞÍ
		LuaFnSetCopySceneData_Param(sceneId,11,542);--É±ËÀtoÕ ğµ ÅÑ¾ü3toÕ ğµ ¹ÖÎïÀàĞÍ
	end	
...
function x311005_OnKillObject( sceneId, selfId, objdataId ,objId )
	...
	killednumber_1 = LuaFnGetCopySceneData_Param(sceneId, 6) ;--É±ËÀÅÑ¾ütoÕ ğµ ÊıÁ¿
	killednumber_2 = LuaFnGetCopySceneData_Param(sceneId, 7) ;--É±ËÀÅÑ¾ütoÕ ğµ ÊıÁ¿
	killednumber_3 = LuaFnGetCopySceneData_Param(sceneId, 8) ;--É±ËÀÅÑ¾ütoÕ ğµ ÊıÁ¿
	killednumber = killednumber_1+killednumber_2+killednumber_3
	IsKillOK=0		
	if killednumber<20 then
		needkilled_1 = LuaFnGetCopySceneData_Param(sceneId, 9) ;--É±ËÀÅÑ¾ütoÕ ğµ ¹ÖÎïÀàĞÍ
		needkilled_2 = LuaFnGetCopySceneData_Param(sceneId,10) ;--É±ËÀÅÑ¾ütoÕ ğµ ¹ÖÎïÀàĞÍ
		needkilled_3 = LuaFnGetCopySceneData_Param(sceneId,11) ;--É±ËÀÅÑ¾ütoÕ ğµ ¹ÖÎïÀàĞÍ
		...
		if IsKillOK==1 then
			...
			LuaFnSetCopySceneData_Param(sceneId, 6, killednumber_1);--ÉèÖÃÉ±ËÀtoÕ ğµ ÅÑ¾ütoÕ ğµ ÊıÁ¿
			LuaFnSetCopySceneData_Param(sceneId, 7, killednumber_2);--ÉèÖÃÉ±ËÀtoÕ ğµ ÅÑ¾ütoÕ ğµ ÊıÁ¿
			LuaFnSetCopySceneData_Param(sceneId, 8, killednumber_3);--ÉèÖÃÉ±ËÀtoÕ ğµ ÅÑ¾ütoÕ ğµ ÊıÁ¿
		end
	end
	...
	if killednumber>=20 and IsKillOK==1 then
		...
		for i=0,num-1 do
			humanObjId = LuaFnGetCopyScene_HumanObjId(sceneId,i)	--È¡ ği¬mµ±Ç°³¡¾°ÀïÈËtoÕ ğµ objId
			misIndex = GetMissionIndexByID(sceneId,humanObjId,x311005_g_MissionId)--È¡ ği¬mÈÎÎñÊı¾İË÷ÒıÖµ
...
function x311005_OnCopySceneTimer( sceneId, nowTime )
	...
	elseif TickCount == x311005_g_LimitTotalHoldTime then --¸±±¾×ÜÊ±¼äÏŞÖÆµ½ÁË
		...
		for	i=0,membercount-1 do
			mems[i] = LuaFnGetCopyScene_HumanObjId(sceneId,i)
			DelMission( sceneId, mems[i], x311005_g_MissionId );--ÈÎÎñth¤t bÕi,É¾³ıÖ®
		...
		for	i=0,membercount-1 do
			...
			if IsHaveMission(sceneId,mems[i],x311005_g_MissionId) > 0 then
				oldteamid = LuaFnGetCopySceneData_Param(sceneId, 12) ; --È¡ ği¬m±£´ætoÕ ğµ ¶ÓÎéºÅ
