...
x311004_g_LimitMembers=1 --¿ÉÒÔ½ø¸±±¾toÕ ğµ ×îĞ¡¶ÓÎéÈËÊı
x311004_g_TickTime=5 --»Øµ÷½Å±¾toÕ ğµ Ê±ÖÓÊ±¼ä(µ¥Î»:  giây/´Î)
x311004_g_LimitTotalHoldTime=120 --¸±±¾¿ÉÒÔ´æ»îtoÕ ğµ Ê±¼ä(µ¥Î»: ´ÎÊı)
x311004_g_CloseTick=6	--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±(µ¥Î»: ´ÎÊı)
x311004_g_NoUserTime=300 --¸±±¾ÖĞÃ»ÓĞÈËºó¿ÉÒÔ¼ÌĞø±£´ætoÕ ğµ Ê±¼ä(µ¥Î»:  giây)
...
function x311004_OnDefaultEvent( sceneId, selfId, targetId )
	if( IsHaveMission(sceneId,selfId,x311004_g_MissionId) > 0)  then	--Èç¹ûÍæ¼ÒÒÑ¾­½ÓÁËCái này ÈÎÎñ
		misIndex = GetMissionIndexByID(sceneId,selfId,x311004_g_MissionId)
		bDone = x311004_CheckSubmit( sceneId, selfId )
		if GetName(sceneId,targetId) == x311004_g_Name then		--ĞúngÄ¿±êNPC
			BeginEvent(sceneId)
    ...
    elseif x311004_CheckAccept(sceneId,selfId) > 0 then		--Ã»ÓĞÈÎÎñ,Thöa mãnÈÎÎñ½ÓÊÕÌõ¼ş
...
function x311004_OnEnumerate( sceneId, selfId, targetId )
	...
	if IsHaveMission(sceneId,selfId,x311004_g_MissionId) > 0 then
		...
		elseif	GetName(sceneId,targetId) ~= x311004_g_Name	and	bDone==1 then --½»ÈÎÎñtoÕ ğµ NPC, ÈÎÎñÍê³É
			AddNumText(sceneId, x311004_g_ScriptId,x311004_g_MissionName);
		end
...
function x311004_OnAccept( sceneId, selfId )
	...
	if( IsHaveMission(sceneId,selfId,x311004_g_MissionId) > 0)  then	--ÒÑ¾­ÓĞÈÎÎñtoÕ ğµ 
		misIndex = GetMissionIndexByID(sceneId,selfId,x311004_g_MissionId)
...
function x311004_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	if x311004_CheckSubmit( sceneId, selfId, selectRadioId )>0 then		--ÒÑ¾­Hoàn t¤t nhi®m vøÁË
...
function x311004_OnKillObject( sceneId, selfId, objdataId ,objId )
	...
	if killednumber>=20 and IsKillOK==1 then
		...
		for i=0,num-1 do
			humanObjId = LuaFnGetCopyScene_HumanObjId(sceneId,i)	--È¡ ği¬mµ±Ç°³¡¾°ÀïÈËtoÕ ğµ objId
			misIndex = GetMissionIndexByID(sceneId,humanObjId,x311004_g_MissionId)--È¡ ği¬mÈÎÎñÊı¾İË÷ÒıÖµ
...
function x311004_OnCopySceneTimer( sceneId, nowTime )
	...
	elseif TickCount == x311004_g_LimitTotalHoldTime then --¸±±¾×ÜÊ±¼äÏŞÖÆµ½ÁË
		...
		for	i=0,membercount-1 do
			mems[i] = LuaFnGetCopyScene_HumanObjId(sceneId,i)
			DelMission( sceneId, mems[i], x311004_g_MissionId );--ÈÎÎñth¤t bÕi,É¾³ıÖ®
