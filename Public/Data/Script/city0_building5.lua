...
x805012_g_PlayerRate = 0.80		-- cáiÈË ðÕt ðßþctoÕ ðµ ×Ê½ð»Ø±¨20%
...
function x805012_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if(guildid ~= cityguildid) then
		BeginEvent(sceneId)
			strText = "    B±n bang tài vø tr÷ng ð¸a# thïnh v§t ð§u lßu thä tÕi hÕ tr÷ng chÑc tÕi thân# di®c b¤t ti®n tiªp ðãi."
	...
	BeginEvent(sceneId)
		...
		if x805012_IsManager(Guildpos)==1 then
			--¹ÜÀíÕß¸£Àû
			AddNumText( sceneId, x805012_g_scriptId, "Lînh phúc lþi bang phái", 7,  9)
			AddNumText( sceneId, x805012_g_scriptId, "Quän lý phúc lþi bang phái", 11, 10 )
		end
...
function x805012_DrawPay( sceneId, selfId  )
	...
	if GetLevel( sceneId, selfId ) < 40 then
		msg = format("ÐÆng c¤p nhân v§t chßa ðü 40.");
	...
	if ((Guildpos ~= GUILD_POSITION_CHIEFTAIN) and (Guildpos ~= GUILD_POSITION_ASS_CHIEFTAIN)) then
		if nCaoYunNum < 2 or nWeek~= nWeekCur then
			msg = format("S¯ l¥n thß½ng nhân trong tu¥n chßa ðü");
			x805012_NotifyTips(sceneId, selfId, msg);
			return 0;
		end
	end
...
function x805012_DrawManagerBonus( sceneId, selfId  )
	...
	if( LuaFnGetGuildAppointTime( sceneId, selfId ) < MIN_APPOINT_TIME_FOR_BONUS ) then
	    msg = format("Các hÕ nên vì anh em trong bang hµi mà góp chút sÑc lñc, sau ðó hãy quay lÕi tìm ta");
		x805012_NotifyTips(sceneId, selfId, msg);
		return 0;
	end
	...
	if GetTodayWeek() ~= 0 then
	    msg = format("Ngày hôm nay không c¤p lþi ích, các hÕ ð×ng vµi");
		x805012_NotifyTips(sceneId, selfId, msg);
		return 0;
	end
...
function x805012_PutGuildMoney( sceneId, selfId, money )
	...
	-- Add BP Point
	local m2p = floor(money/10000)
	if (m2p > 0) then
		CityChangeAttr(sceneId, selfId, GUILD_CONTRIB_POINT, m2p)
		x805012_NotifyTips(sceneId, selfId, "Nh§n ðßþc "..m2p.." ði¬m c¯ng hiªn bang hµi")
	end
