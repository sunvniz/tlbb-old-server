...
x714093_g_eventList={20104001,20104002,20104005,20104008,20104009,20104010,
			20105001,20105003,20105006,20105009,20105010} --Ö²ÎïtoÕ ðµ ±àºÅ,²»ÐúngscriptId
--**********************************
--ÊÂ¼þ½»»¥Èë¿Ú
--**********************************
...
function x714093_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if NumText == 254 or NumText == 255 then
		BeginEvent(sceneId)
			...
			--Í¨¹ýx714093_g_eventListºÍscriptglobalÖÐtoÕ ðµ Ö²ÎïÁÐ±í¶Ô±È,²¢¸ù¾ÝÍæ¼ÒÖÖÖ²¼¼ÄÜµÈc¤pÀ´ÏÔÊ¾ÏàÓ¦Ö²Îï
			for i, eventId in x714093_g_eventList do	--±éÀúCái này µ¾²ÝÈË¿ÉÒÔÖÖÖ²toÕ ðµ Ö²ÎïÁÐ±í
				for j,g_ZhiWuId in V_ZHONGZHI_ID do		--±éÀúscriptglobalÖÐtoÕ ðµ ËùÓÐÖ²ÎïÁÐ±í
					if eventId == g_ZhiWuId then
						...
						if AbilityLevel >= V_ZHONGZHI_NEEDLEVEL[j] then --Èç¹ûÍæ¼ÒÖÖÖ²¼¼ÄÜµÈc¤p>=¸ÃÖ²ÎïÒªÇó¼¼ÄÜµÈc¤p]
							...
							AddNumText(sceneId, x714093_g_ScriptId, V_ZHONGZHI_NAME[name_Index],6,name_Index)
							break
						end
					end
				end
