...
function x714021_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if AbilityLevel ~= 0	then
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x714021_g_ScriptId, "Tr°ng loÕi thu hoÕch muµn",6,255)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
end
...
function x714021_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if NumText == 254 or NumText == 255 then
		BeginEvent(sceneId)
			...
			--Í¨¹ýx714021_g_eventListºÍscriptglobalÖÐtoÕ ðµ Ö²ÎïÁÐ±í¶Ô±È,²¢¸ù¾ÝÍæ¼ÒÖÖÖ²¼¼ÄÜµÈc¤pÀ´ÏÔÊ¾ÏàÓ¦Ö²Îï
			for i, eventId in x714021_g_eventList do	--±éÀúCái này µ¾²ÝÈË¿ÉÒÔÖÖÖ²toÕ ðµ Ö²ÎïÁÐ±í
				for j,g_ZhiWuId in V_ZHONGZHI_ID do		--±éÀúscriptglobalÖÐtoÕ ðµ ËùÓÐÖ²ÎïÁÐ±í
					if eventId == g_ZhiWuId then
						if AbilityLevel >= V_ZHONGZHI_NEEDLEVEL[j] then --Èç¹ûÍæ¼ÒÖÖÖ²¼¼ÄÜµÈc¤p>=¸ÃÖ²ÎïÒªÇó¼¼ÄÜµÈc¤p]
							...
							AddNumText(sceneId, x714021_g_ScriptId, V_ZHONGZHI_NAME[name_Index],6,name_Index)
							break
						end
					end
				end
