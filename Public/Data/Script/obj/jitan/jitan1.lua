...
function x900007_OnDefaultEvent( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x900007_g_scriptId, "Ð£t 10 cái #GCØu lê phø tùng", 6, 10)
		AddNumText( sceneId, x900007_g_scriptId, "V« Ác chiªn cØu lê", 11, 11)
		--AddNumText( sceneId, x900007_g_ScriptId, "V« Truy tìm Næ Oa thÕch", 11, 12 )
		--AddNumText( sceneId, x900007_g_scriptId, "V« Truy tìm Næ Oa th¥n thÕch", 11, 13)
        EndEvent(sceneId)
...
function x900007_OnEventRequest( sceneId, selfId, targetId, eventId )
  ...
  if GetNumText()  == 10 then
	...
	LuaFnDelAvailableItem(sceneId,selfId,x900007_g_ItemId,10)	-- Xóa v§t ph¦m
	--É¾³ý¸ÃÎïÆ·
	...
	if LuaFnLockCheck( sceneId, selfId, bagpos, 0 ) < 1 then
		...
		if nItemNum <= 0 then
			x900007_MsgBox( sceneId, selfId, targetId, "V§t ph¦m ðã b¸ khóa!" )
			return 0;
		end
	end
...
--¿ªÊ¼Ö´ÐÐËæ»úÊÂ¼þ
		...
		local strText = format("#b#cff00f0Thái khôn cØu lê ð¥u lînh: Là ai, dám qu¤y r¥y ta ngü, các ngß½i s¨ phäi trä giá ð¡c cho vi®c này!", playername)		
