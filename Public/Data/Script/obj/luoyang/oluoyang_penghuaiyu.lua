...
function x000110_OnDefaultEvent( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		...
		--AddNumText( sceneId, x000110_g_scriptId, "TEST - Nh§n S½ c¤p dung luy®n phù", 6, 69 )
		...
		AddNumText( sceneId, x000110_g_scriptId, "#{BSQKY_20110506_09}", 6, 16 ) -- add by Dark.Hades 20170131
...
function x000110_OnEventRequest( sceneId, selfId, targetId, eventId )
         ...
         if GetNumText() == 69 then
		 TryRecieveItem(sceneId, selfId, 30900056, 1)
		 end
		---------------------------½éÉÜ-------------------------------------------------
...
--				AddNumText(sceneId, x000110_g_scriptId,"×°±¸ÐÞÀí½éÉÜ",11,12);
				...
				AddNumText(sceneId, x000110_g_scriptId,"#{BSQKY_20110506_10}",11,17);
		...
		if GetNumText() == 17 then
			BeginEvent(sceneId)
				AddText(sceneId,"#{BSQKY_20110506_25}#r")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
			return
		end
