...
function x014012_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		AddNumText(sceneId, x014012_g_ScriptId, "H÷c t§p khinh công phái Tiêu Dao",12,0)
	EndEvent(sceneId)
...
function x014012_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText()==0	then
		...
		elseif GetMenPai(sceneId, selfId) == 9 then
			...
			BeginEvent(sceneId)
				AddText(sceneId,"Ngß½i không phäi ð® tØ cüa phái Tiêu Dao, ta không th¬ truy«n dÕy ngß½i khinh công cüa b±n môn.")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
	end	
