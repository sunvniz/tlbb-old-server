--Éú³¤ ği¬m
--¶ÔÓ¦Éú»î¼¼ÄÜ: -1
--ÑàÍõ¹ÅÄ¹ÑàÏÒÓñµôÂä
--½Å±¾ºÅ******(712528)

--mµt cái±¦ÏäÀïº¬ÓĞ¼¸cáiÎïÆ·
x712528_g_DropNumTable  = {
	{num = 1, odd = 1},	--1cáitoÕ ğµ ¼¸ÂÊ
	{num = 2, odd = 0},	--2cáitoÕ ğµ ¼¸ÂÊ
	{num = 3, odd = 0},	--3cáitoÕ ğµ ¼¸ÂÊ
	{num = 4, odd = 0},	--4cáitoÕ ğµ ¼¸ÂÊ
	{num = 5, odd = 0},	--5cáitoÕ ğµ ¼¸ÂÊ
}

x712528_g_TickCreate_Msg = ""

--Ã¿ÖÖÎïÆ·ĞúngÄÄÀàÎïÆ·,ÇÒ´ËÀàÎïÆ·¶ÑµşÊıÁ¿Ğúng¼¸cái
x712528_g_DropTable = {
	--ÑªÒ©Àà
	{
		itemType = 1, 	--ÑªÒ©ÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 0.2},				--1cáitoÕ ğµ ¼¸ÂÊ
							{num = 2, odd = 0.4},				--2cáitoÕ ğµ ¼¸ÂÊ
							{num = 3, odd = 0.2},				--3cáitoÕ ğµ ¼¸ÂÊ
							{num = 4, odd = 0.15},			--4cáitoÕ ğµ ¼¸ÂÊ
							{num = 5, odd = 0.05},			--5cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						30001004,30003007,30101037,
					},
	},
	--À¶Ò©Àà
	{
		itemType = 2, 	--À¶Ò©ÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 0.2},				--1cáitoÕ ğµ ¼¸ÂÊ
							{num = 2, odd = 0.4},				--2cáitoÕ ğµ ¼¸ÂÊ
							{num = 3, odd = 0.2},				--3cáitoÕ ğµ ¼¸ÂÊ
							{num = 4, odd = 0.15},			--4cáitoÕ ğµ ¼¸ÂÊ
							{num = 5, odd = 0.05},			--5cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						30002004,30101047,
					},
	},
	--¿ÚÁ¸
	{
		itemType = 3, 	--¿ÚÁ¸toÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 0.2},				--1cáitoÕ ğµ ¼¸ÂÊ
							{num = 2, odd = 0.4},				--2cáitoÕ ğµ ¼¸ÂÊ
							{num = 3, odd = 0.2},				--3cáitoÕ ğµ ¼¸ÂÊ
							{num = 4, odd = 0.15},			--4cáitoÕ ğµ ¼¸ÂÊ
							{num = 5, odd = 0.05},			--5cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						30602007,30603007,30604007,30605007,
					},
	},
	--²ÄÁÏÀà
	{
		itemType = 4, 	--²ÄÁÏÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 1, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1},				--2cáitoÕ ğµ ¼¸ÂÊ
							{num = 4, odd = 0},				--4cáitoÕ ğµ ¼¸ÂÊ
							{num = 6, odd = 0},				--6cáitoÕ ğµ ¼¸ÂÊ
							{num = 8, odd = 0},			--8cáitoÕ ğµ ¼¸ÂÊ
							{num = 10, odd = 0},			--10cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						20310000,
					},
	},
	--°éÉúÀà
	{
		itemType = 12, 	--°éÉúÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 0.3},				--1cáitoÕ ğµ ¼¸ÂÊ
							{num = 2, odd = 0.4},				--2cáitoÕ ğµ ¼¸ÂÊ
							{num = 3, odd = 0.3},				--3cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						20102031,20103019,20103031,20103043,20103055,20105019,
					},
	},
	--°×É«ÎäÆ÷Àà
	{
		itemType = 5, 	--°×É«ÎäÆ÷ÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						10100007,10101007,10102007,10103007,10104007,10105007,
					},
	},
	--À¶É«ÎäÆ÷Àà
	{
		itemType = 10, 	--À¶É«ÎäÆ÷ÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						 10200007,10201007,10202007,10203007,10204007,10205007,
					},
	},
	--°×É«×°±¸Àà
	{
		itemType = 6, 	--°×É«×°±¸ÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						10110018,10110019,10110020,10110021,10110022,10110023,10111018,10111019,10111020,10111021,10111022,10111023,10112018,10112019,10112020,10112021,10112022,10112023,10113018,10113019,10113020,10113021,10113022,10113023,10120007,10121007,10122007,
					},
	},
	--À¶É«×°±¸Àà
	{
		itemType = 11, 	--À¶É«×°±¸ÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						 10210013,10211013,10212013,10213013,10220007,10221007,10222007,
					},
	},
	--±¦Ê¯Àà
	{
		itemType = 7, 	--±¦Ê¯ÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						50101001,50101002,50102001,50102002,50102003,50102004,50103001,50104002,50111001,50111002,50112001,50112002,50112003,50112004,50113001,50113002,50113003,50113004,50113005,50114001,
					},
	},
	--Åëâ¿Åä·½Àà
	{
		itemType = 8, 	--Åëâ¿Åä·½ÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						
					},
	},
	--ÖÆÔìÅä·½Àà
	{
		itemType = 13, 	--Åëâ¿Åä·½ÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						30302200 ,30302205	,30302210	,30302215	,30302220	,30302225	,30302230	,30302235	,30302240	,30302245	,30302250	,30302255	,30302260	,30302265	,30302270	,30302275	,30302280	,30302285	,30302290	,30302295	,30302300	,30302305	,30302310	,30302315,
					},
	},
	--ÕäÊŞ
	{
		itemType = 9, 	--ÕäÊŞtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = -1.0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 0.0},				--1cáitoÕ ğµ ¼¸ÂÊ
						 },
		idx = {	--ÕäÊŞĞòÁĞºÅ±í
						3009,3019,3029,
					},
	},
}

--±¦Îï»ØÊÕÊ±¼ä2006-8-21 17:33 ĞŞ¸ÄÎªÓÉ¹ÜÀíÆ÷´«µİ½øÀ´

--¸ù¾İ±¾´ÎËæ»úÊıÀ´ÕÒµ½±íÖĞ¶ÔÓ¦toÕ ğµ Ë÷Òı
function x712528_GetTableIndexByOdd(tb)
	local oddNow = random();
	
	local base = 0.0;
	for i = 1, getn(tb) do
		if(tb[i].odd + base >= oddNow) then
			return i;
		end
		base = base + tb[i].odd;
	end
	
	return nil;
end

--Éú³Éº¯Êı¿ªÊ¼************************************************************************
--Ã¿cáiItemBoxÖĞ×î¶à10cáiÎïÆ·,Tr· v«Öµ -1 Ö±½Ó»ØÊÕÉú³¤ ği¬m,²»Õ¼ÓÃ
function x712528_OnCreate( sceneId, growPointType, x, y, dur)
	local ItemBoxId = nil
	local delDur = dur - 60000;	--±¦Ïä»òÕäÊŞtoÕ ğµ ÏûÊ§Ê±¼ä
	
	-- ±¦ÏäÀïÓĞ¼¸cáiÎïÆ·
	local numIdx = x712528_GetTableIndexByOdd(x712528_g_DropNumTable)
	if(numIdx) then
		for i = 1, x712528_g_DropNumTable[numIdx].num do
			--Cái này ÎïÆ·ĞúngÊ²Ã´ÀàĞÍtoÕ ğµ ÎïÆ·
			local showIdx = x712528_GetTableIndexByOdd(x712528_g_DropTable)
			local dropNumIdx = nil
			local itemId = nil
			if(showIdx) then
				--Cái này ÀàĞÍtoÕ ğµ ÎïÆ·ÊıÁ¿Ğúng¼¸cái
				dropNumIdx = x712528_GetTableIndexByOdd(x712528_g_DropTable[showIdx].numOdd)
				if(dropNumIdx and 9 ~= x712528_g_DropTable[showIdx].itemType) then
					local itemNum = getn(x712528_g_DropTable[showIdx].idx)
					if(itemNum and 1<=itemNum) then
						local itemIdx = floor(random(1, itemNum))
						itemId = x712528_g_DropTable[showIdx].idx[itemIdx]
						--Ìí¼ÓµôÂäÏäºÍÎïÆ·µ½³¡¾°
						for k = 1, x712528_g_DropTable[showIdx].numOdd[dropNumIdx].num do
							if(nil == ItemBoxId) then
								ItemBoxId = ItemBoxEnterScene( x, y, growPointType, sceneId, QUALITY_MUST_BE_CHANGE,1, itemId )
								print( sceneId .. " Trß¶ng cänh s¯ (" .. x .. ", " .. y .. ") tÕo ra mµt v§t ph¦m "..
											 "("..x712528_g_DropNumTable[numIdx].num..")" )
							else
								AddItemToBox( sceneId, ItemBoxId, QUALITY_MUST_BE_CHANGE,1, itemId )
							end
						end
					end
				elseif(dropNumIdx and 9 == x712528_g_DropTable[showIdx].itemType) then
					local petIdx = floor(random(1, getn(x712528_g_DropTable[showIdx].idx)))
					local petId = x712528_g_DropTable[showIdx].idx[petIdx]
					--Ìí¼ÓÕäÊŞµ½³¡¾°
					for k = 1, x712528_g_DropTable[showIdx].numOdd[dropNumIdx].num do
						local PetObjId = nil
						PetObjId = CreatePetOnScene( sceneId, petId, x, y )
						SetCharacterDieTime( sceneId, PetObjId, delDur )
						print( sceneId .. " Trß¶ng cänh s¯ (" .. x .. ", " .. y .. ") tÕo ra mµt Trân thú "..
									 GetName( sceneId, PetObjId ) .. "." )
					end 
				end
			end --end of if(showIdx) then
			
			--¼ÇÂ¼ÎïÆ·µôÂä°üËù°üº¬toÕ ğµ ÄÚÈİ
			if(showIdx and dropNumIdx and itemId ~= nil) then
				if(9 ~= x712528_g_DropTable[showIdx].itemType) then
					local itemName;
					_,itemName,_ = GetItemInfoByItemId(itemId)
					print("V§t ph¦m bên trong có (loÕi "..x712528_g_DropTable[showIdx].itemType..")"..
								x712528_g_DropTable[showIdx].numOdd[dropNumIdx].num..
								" mµt ["..itemName.."].")
					itemNum = x712528_g_DropTable[showIdx].numOdd[dropNumIdx].num;
					LuaFnAuditItemCreate(sceneId, -1, itemNum, itemId, itemName, "T× bäo sß½ng tÕo ra");
				end
			end
			
		end -- end of for i = 1, x712528_g_DropNumTable[numIdx].num do
		
		--Éè¶¨»ØÊÕÊ±¼ä
		if(ItemBoxId) then
			SetItemBoxMaxGrowTime( sceneId, ItemBoxId, delDur )
			return 0	--Cái này ×ø±ê±»±¦ÏäÕ¼ÓÃÁË
		end
	end
	
	return -1	--Cái này ×ø±êÃ»ÓĞ±»Õ¼ÓÃ
end
--Éú³Éº¯Êı½áÊø**********************************************************************

--mµt ´Î´´½¨¶àcái±¦ÏätoÕ ğµ Íê³Éº¯Êı¿ªÊ¼****************************************************
function x712528_OnTickCreateFinish( sceneId, growPointType, tickCount )
	if(strlen(x712528_g_TickCreate_Msg) > 0) then
		--2006-8-22 14:37 µÈ´ıÏş½¡toÕ ğµ server¶Ô»°Æ½Ì¨
		print( sceneId .. " Trß¶ng cänh s¯ "..x712528_g_TickCreate_Msg)
	end
end
--mµt ´Î´´½¨¶àcái±¦ÏätoÕ ğµ Íê³Éº¯Êı½áÊø****************************************************

--´ò¿ªÇ°º¯Êı¿ªÊ¼
function x712528_OnOpen( sceneId, selfId, targetId )
--Tr· v«ÀàĞÍ
-- 0 ±íÊ¾´ò¿ª³É¹¦
end
--´ò¿ªÇ°º¯Êı½áÊø


--»ØÊÕº¯Êı¿ªÊ¼
function x712528_OnRecycle( sceneId, selfId, targetId )
--Tr· v«1,Éú³¤ ği¬m»ØÊÕ
	return 1
end
--»ØÊÕº¯Êı½áÊø

--´ò¿ªºóº¯Êı¿ªÊ¼
function x712528_OnProcOver( sceneId, selfId, targetId )
end

--´ò¿ªºóº¯Êı½áÊø
function x712528_OpenCheck( sceneId, selfId, AbilityId, AblityLevel )
	return OR_OK
end
