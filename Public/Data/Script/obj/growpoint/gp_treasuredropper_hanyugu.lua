...
x712532_g_DropTable = {
	...
	--¸ß¼ÛÖµÆæÎï
	{
		itemType = 3, 	--°éÉúÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0.01, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
	...
	--Ò×Èİµ¤
	{
		itemType = 7, 	--°éÉúÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0.04, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1cáitoÕ ğµ ¼¸ÂÊ
						 },
	...
	--¹ÅÄ¹ĞĞ¹¦Yªu Quyªt
	{
		itemType = 8, 	--°éÉúÀàtoÕ ğµ ÀàĞÍ±êÊ¶
	 	odd = 0.01, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1cáitoÕ ğµ ¼¸ÂÊ
						 },
		...
		for i = 1, x712532_g_DropNumTable[numIdx].num do
			...
			if(showIdx and dropNumIdx and itemId ~= nil) then
				if(9 ~= x712532_g_DropTable[showIdx].itemType) then
					...
					print("V§t ph¦m bên trong có (loÕi "..x712532_g_DropTable[showIdx].itemType..")"..
...
function x712532_OnTickCreateFinish( sceneId, growPointType, tickCount )
	if(strlen(x712532_g_TickCreate_Msg) > 0) then
		...
		print( sceneId .. " Trß¶ng cänh s¯ "..x712532_g_TickCreate_Msg)
	end
end
