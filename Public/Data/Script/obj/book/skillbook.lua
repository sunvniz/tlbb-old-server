...
function x338000_OnConditionCheck( sceneId, selfId )
	...
	if -1~=skillBook.menpaiId then
	 if menpai ~= skillBook.menpaiId then
		 x338000_NotifyFailTips( sceneId, selfId, "Ngß½i không phäi #{_MENPAI" .. skillBook.menpaiId .. "} ð® tØ, không th¬ h÷c " .. x338000_g_TypeNames[skillBook.type] .. ". " )
		 return 0
	 end
	end

	if GetLevel( sceneId, selfId ) < skillBook.needLevel then
		x338000_NotifyFailTips( sceneId, selfId, "Ngß½i ðü c¤p ðµ ð¬ h÷c " .. x338000_g_TypeNames[skillBook.type] .. ". " )
		return 0
	end
	...
	if skillBook.needXinfa ~= -1 then
		...
		if xinfaLevel < 1 then
			x338000_NotifyFailTips( sceneId, selfId, "C¥n Bí t¸ch: #{_XINFA" .. skillBook.needXinfa .. "}. " )
			return 0
		end
		...
		if xinfaLevel < skillBook.needXinfaLevel then
			...
			x338000_NotifyFailTips( sceneId, selfId, "C¥n Bí t¸ch: #{_XINFA" .. skillBook.needXinfa .. "} "
				.. skillBook.needXinfaLevel .. " hi®n ngß½i ðang ðÕt c¤p " .. xinfaLevel .. ". " )
			return 0
		end
	end
	...
	if skillBook.type == 1 then					-- ÐÄ·¨
		if HaveXinFa( sceneId, selfId, skillBook.id ) > 0 then
			x338000_NotifyFailTips( sceneId, selfId, "Ngß½i ðã h÷c qua tâm pháp này" )
			return 0
		end
