...
function x894072_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		if 1 == CallScriptFunction( x894070_g_FuBenScriptId, "GetBossBattleFlag", sceneId, "YeLvYan" ) then
		    AddNumText( sceneId, x894072_g_ScriptId, "Khiêu chiªn", 10, 1 )
		end
...
function x894072_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if 2 ~= CallScriptFunction( x894072_g_FuBenScriptId, "GetBossBattleFlag", sceneId, "XiaoRuJun" ) then
		BeginEvent(sceneId)
			AddText( sceneId, "Huynh ð®, hãy cùng ta chiªn ð¤u." )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end

	if 2 ~= CallScriptFunction( x894072_g_FuBenScriptId, "GetBossBattleFlag", sceneId, "ShuangZi" ) then
		BeginEvent(sceneId)
			AddText( sceneId, "Huynh ð®, hãy cùng ta chiªn ð¤u." )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
...
function x894072_OnBQZTimer( sceneId, step, data1, data2 )
	...
	if 7 == step then
		CallScriptFunction( x894072_g_FuBenScriptId, "TipAllHuman", sceneId, "Chiªn ð¤u 5 giây sau b¡t ð¥u." )
		return
	end

	if 6 == step then
		CallScriptFunction( x894072_g_FuBenScriptId, "TipAllHuman", sceneId, "Chiªn ð¤u 4 giây sau b¡t ð¥u." )
		return
	end

	if 5 == step then
		CallScriptFunction( x894072_g_FuBenScriptId, "TipAllHuman", sceneId, "Chiªn ð¤u 3 giây sau b¡t ð¥u." )
		return
	end

	if 4 == step then
		CallScriptFunction( x894072_g_FuBenScriptId, "TipAllHuman", sceneId, "Chiªn ð¤u 2 giây sau b¡t ð¥u." )
		return
	end

	if 3 == step then
		CallScriptFunction( x894072_g_FuBenScriptId, "TipAllHuman", sceneId, "Chiªn ð¤u 1 giây sau b¡t ð¥u." )
		return
	end

	if 2 == step then
		--ÌáÊ¾Chiªn ð¤u b¡t ð¥u.....
		CallScriptFunction( x894072_g_FuBenScriptId, "TipAllHuman", sceneId, "Chiªn ð¤u b¡t ð¥u." )
		--É¾³ýNPC....
