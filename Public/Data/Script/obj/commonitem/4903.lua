...
--½Å±¾ºÅ
x334903_g_scriptId = 334903 --ÁÙÊ±Ğ´Cái này ,ÕæÕıÓÃtoÕ ğµ Ê±ºòmµt ¶¨Òª¸Ä.
...
x334903_g_standFlag = 1 -- 2:¶ÓÓÑ, 1: ÓÑ¾ü, -1: µĞ¾ü
--AEÓ°ÏìÊıÄ¿ÏŞÖÆ
x334903_g_effectCount = 4 -- -1:²»ÏŞÖÆ
--Ğ§¹ûtoÕ ğµ ID
x334903_g_Impact1 = 4903 --ÁÙÊ±Ğ´Cái này 
x334903_g_Impact2 = -1 --²»ÓÃ
...
--		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x334903_g_Impact1, 0);
		...
		if(0<=targetId) then
			if LuaFnIsFriend(sceneId, targetId, selfId) > 0 then
				...
				BeginEvent(sceneId)
			    AddText(sceneId, "ÄãÓë¶Ô·½toÕ ğµ ÓÑºÃ¶ÈÔö¼ÓÁË70");
			  EndEvent(sceneId)
