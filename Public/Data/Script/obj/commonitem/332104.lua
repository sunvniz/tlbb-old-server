...
function x332104_OnActivateOnce( sceneId, selfId )
    ...
    if PlayerHP == PlayerMaxHP then
        ...
        if NeedHP >= ValidValue then  --¿ÉÌá¹©toÕ ğµ HP²»×ã»Ö¸´Íæ¼Ò¶¨Á¿HP
            ...
            SetBagItemParam( sceneId, selfId, bagId, 4, 2, x332104_g_MaxValue ) --ÉèÖÃ×î´óÈİÁ¿
            SetBagItemParam( sceneId, selfId, bagId, 8, 2, x332104_g_MaxValue ) --ÉèÖÃÒÑÓÃÈİÁ¿
            bErased = EraseItem( sceneId, selfId, bagId )
        else
            IncreaseHp(sceneId, selfId, NeedHP ) --»Ö¸´C¥n toÕ ğµ HP
            SetBagItemParam( sceneId, selfId, bagId, 4, 2, x332104_g_MaxValue ) --ÉèÖÃ×î´óÈİÁ¿
            SetBagItemParam( sceneId, selfId, bagId, 8, 2, UseValue + x332104_g_IncPerAct ) --ÉèÖÃÒÑÓÃÈİÁ¿
        ...
        if bErased < 0 then
			...
			local szLog = "Use31000006, Tên=%s, Guid=%d, Lßşng dßşc sØ døng trß¾c=%d, Lßşng dßşc sØ døng sau=%d, SØ døng trß¾c HP=%d, sØ døng sau HP=%d, v¸ trí tay näi=%d"
			local UseValue_log = GetBagItemParam( sceneId, selfId, bagId, 8, 2 )
			local nCA = x332104_g_MaxValue - UseValue_log   --sØ døng sau ÈİÁ¿
			
			local szDebugLog = format( szLog, szName, nGuid, nCB, nCA, nHPB, nHPA, bagId )
			WriteDebugLog( sceneId, selfId, szDebugLog )
        else
			local szLog = "Use31000006, Tên=%s, Guid=%d, Lßşng dßşc sØ døng trß¾c=%d, Ğã b¸ c¡t bö, SØ døng trß¾c HP=%d, sØ døng sau HP=%d, v¸ trí tay näi=%d"
			local szDebugLog = format( szLog, szName, nGuid, nCB, nHPB, nHPA, bagId )
			WriteDebugLog( sceneId, selfId, szDebugLog )
        end
        
        --LuaFnSendSpecificImpactToUnit( sceneId, selfId, selfId, selfId, 18, 1000 )
        --SetMp( sceneId, selfId, 0 ) --²âÊÔÀ¶Ò©ÓÃ´úÂë,mµt ¶¨ÒªÉ¾ÁË°¡!
