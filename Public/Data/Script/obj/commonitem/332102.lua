...
function x332102_OnActivateOnce( sceneId, selfId )
    ...
    if PlayerMP == PlayerMaxMP then
		...
		local nMPB = PlayerMP    --SØ døng trß¾c Íæ¼Òhp
        ...
        if bErased < 0 then
			...
			local szLog = "Use31000002, Tên=%s, Guid=%d, Lßşng dßşc sØ døng trß¾c=%d, Lßşng dßşc sØ døng sau=%d, SØ døng trß¾c MP=%d, sØ døng sau MP=%d, v¸ trí tay näi=%d"
			local UseValue_log = GetBagItemParam( sceneId, selfId, bagId, 8, 2 )
			local nCA = x332102_g_MaxValue - UseValue_log   --sØ døng sau ÈİÁ¿
			
			local szDebugLog = format( szLog, szName, nGuid, nCB, nCA, nMPB, nMPA, bagId )
			WriteDebugLog( sceneId, selfId, szDebugLog )  
        else
			local szLog = "Use31000002, Tên=%s, Guid=%d, Lßşng dßşc sØ døng trß¾c=%d, Ğã b¸ c¡t bö, SØ døng trß¾c MP=%d, sØ døng sau MP=%d, v¸ trí tay näi=%d"
			local szDebugLog = format( szLog, szName, nGuid, nCB, nMPB, nMPA, bagId )
			WriteDebugLog( sceneId, selfId, szDebugLog )
        end
        
        --LuaFnSendSpecificImpactToUnit( sceneId, selfId, selfId, selfId, 18, 1000 )
        --SetHp( sceneId, selfId, 1 ) --²âÊÔÀ¶Ò©ÓÃ´úÂë,mµt ¶¨ÒªÉ¾ÁË°¡!
