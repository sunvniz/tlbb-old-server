...
--½Å±¾ºÅ
x334915_g_scriptId = 334915 --ÁÙÊ±Ğ´Cái này ,ÕæÕıÓÃtoÕ ğµ Ê±ºòmµt ¶¨Òª¸Ä.
...
--		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x334915_g_Impact1, 0);
		...
		if(0<=targetId) then
			if LuaFnIsFriend(sceneId, targetId, selfId) > 0 then
				...
				BeginEvent(sceneId)
			    AddText(sceneId, "ÄãÓë¶Ô·½toÕ ğµ ÓÑºÃ¶ÈÔö¼ÓÁË5");
			  EndEvent(sceneId)
			  DispatchMissionTips(sceneId,selfId)
			  
			  local	namSelf		= GetName( sceneId, selfId )
			  local	namTarget	= GetName( sceneId, targetId )
				local	str	= format( "#B#{_INFOUSR"..namSelf.."}#cffffffÇ×ÊÖ°Ñ#W[1¶äÃµ¹å]#cffffffËÍµ½#B#{_INFOUSR"..namTarget.."}#cffffffÊÖÖĞ,ÉîÇéµØ¿´×Å#B#{_INFOUSR"..namTarget.."}#cffffff." )
