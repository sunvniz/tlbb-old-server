...
--½Å±¾ºÅ
 x887777_g_scriptId = 887777 --ÁÙÊ±Ð´Cái này ,ÕæÕýÓÃtoÕ ðµ Ê±ºòmµt ¶¨Òª¸Ä.
...
--×¢: Á½cáiÆïÊõ»ámµt cái¾Í¿ÉÒÔÆïÁË
 x887777_g_Equitation1 = 1109 --C¥n toÕ ðµ ÆïÊõ
 x887777_g_Equitation2 = -1 --C¥n toÕ ðµ ÆïÊõ
--Ð§¹ûtoÕ ðµ ID
 x887777_g_Impact1 = 1 --ÁÙÊ±Ð´Cái này
 x887777_g_Impact2 = -1 --²»ÓÃ
...
function  x887777_CancelImpacts( sceneId, selfId )
	nRet = 0;
	nRet = LuaFnCancelSpecificImpact(sceneId, selfId,  x887777_g_Impact1)
	if(0<nRet) then
		return 1;
	end
	return 0;
...
function  x887777_OnConditionCheck( sceneId, selfId )
	...
	if(-1~= x887777_g_Equitation1) then
		if(1==LuaFnHaveSpecificEquitation(sceneId, selfId,  x887777_g_Equitation1)) then
			return 1
		end
	end
	if(-1~= x887777_g_Equitation2) then
		if(1==LuaFnHaveSpecificEquitation(sceneId, selfId,  x887777_g_Equitation2)) then
			return 1
		end
	end
...
function  x887777_OnActivateOnce( sceneId, selfId )
	if(-1~= x887777_g_Impact1) then
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId,  x887777_g_Impact1, 0)
	end
