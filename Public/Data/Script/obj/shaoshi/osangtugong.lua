...
function x890071_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		if 1 == CallScriptFunction( x890070_g_FuBenScriptId, "GetBossBattleFlag", sceneId, "ZhangJuXian" ) then
		    AddNumText( sceneId, x890071_g_ScriptId, "ÌôÕ½", 10, 2 )
		end
   ...
   if GetNumText() == 2 then
	BeginEvent(sceneId)
		AddText(sceneId,"      ÄãÈ·ÈÏ¿ªÊ¼ÌôÕ½Ã´£¿¿ªÊ¼ÌôÕ½ºó£¬ËÂÃÅ½«±»·âÓ¡£¬ËÀÍöºó½«ÎŞ·¨Á¢¼´²ÎÓëÕ½¶·£»ËùÓĞ¶ÓÔ±ÍÑÀëÕ½¶·£¬·âÓ¡½«½â³ı¡£")
		    AddNumText( sceneId, x890071_g_ScriptId, "ÌôÕ½", 10, 1 )
	EndEvent(sceneId)
	DispatchEventList(sceneId,selfId,targetId)
  end
...
function x890071_OnSSSTimer( sceneId, step, data1, data2 )
	...
	if 7 == step then
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "Õ½¶·5ÃëÖÓºó¿ªÊ¼" )
		return
	end

	if 6 == step then
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "Õ½¶·4ÃëÖÓºó¿ªÊ¼" )
		return
	end

	if 5 == step then
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "Õ½¶·3ÃëÖÓºó¿ªÊ¼" )
		return
	end

	if 4 == step then
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "Õ½¶·2ÃëÖÓºó¿ªÊ¼" )
		return
	end

	if 3 == step then
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "Õ½¶·1ÃëÖÓºó¿ªÊ¼" )
		return
	end

	if 2 == step then
		--ÌáÊ¾Õ½¶·¿ªÊ¼....
		CallScriptFunction( x890071_g_FuBenScriptId, "TipAllHuman", sceneId, "Õ½¶·¿ªÊ¼" )
		--É¾³ıNPC....
