...
x890012_g_itemId = 39901017
x890012_g_MonsterId = 39712
...
function x890012_UpdateEventList( sceneId, selfId,targetId )		 
	...
	if not level or level < 21 then
	  ...
	  if LuaFnDelAvailableItem(sceneId, selfId, x890012_g_itemId, 1) == 0 then
		BeginEvent(sceneId)
		...
		nObjID = LuaFnCreateMonster(sceneId, x890012_g_MonsterId, posX, posZ, 1, 253, 0);
		if nObjID and nObjID ~= -1 then
		--	SetCharacterDieTime(sceneId, nObjID, 600000);
			SetCharacterTitle(sceneId, nObjID, "牛奇");
		--	LuaFnSetMonsterExp(sceneId, nObjID, 0);
		--	LuaFnDisableMonsterDropBox(sceneId, nObjID);
		end
            local  nam= LuaFnGetName( sceneId, selfId )
		  local strText = format ("#b#cff00f0恭喜玩家★★#c00ff00"..nam.."#b#cff00f0★★成功使用BOSS卡片召唤出牛奇,本服BOSS卡片召唤BOSS消灭后可爆出赠点以及石头宝箱,手工材料制作等高级道具!#Y", nam)						
