...
--Ð¡°×ÍÃ¶ú¶äÊý¾Ý±í....µÚ50»·Ê±C¥n ¸ÄËÍÐ¡°×ÍÃ¶ú¶ä....
...
function x005001_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then
		...
		--¿´ÏÂ½ñÌìtoÕ ðµ ÈÎÎñÐúng²»Ðúng×öÍê50´ÎÁË
...
function x005001_OnModEvent_Submit(sceneId, selfId, targetId, missionIndex)
	...
	--Èç¹ûÐúngµÚ50»·(µÚmµt ´Î¸ø)....ÔòËÍÍæ¼Òmµt cáiÐ¡°×ÍÃ¶ú¶ä....·ñÔò¾ÍËÍÆÕÍ¨ÍÃ¶ú¶ä....
	if count == 50 then
...
function x005001_GiveRabbitEar( sceneId, selfId )
	...
	if ret and ret > 0 then
		...
		if itemTransfer and selfName then
			local msg = "#YNguy­n Tinh Trúc: #W#{_INFOUSR"..selfName.."}#P#{tuerduo}#{_INFOMSG"..itemTransfer.."}#P ðem lÕi #W#{_INFOUSR"..selfName.."}#Pbày tö lòng biªt ½n!";
...
function x005001_GiveSmallRabbitEar( sceneId, selfId )
	...
	if ret and ret > 0 then
		...
		if itemTransfer and selfName then
			local msg = "#YNguy­n Tinh Trúc: #W#{_INFOUSR"..selfName.."}#P#{tuerduo2}#{_INFOMSG"..itemTransfer.."}#P ðßa cho #W#{_INFOUSR"..selfName.."}#P#{tuerduo3}";
			BroadMsgByChatPipe(sceneId, selfId, msg, 4);
		end
	end
