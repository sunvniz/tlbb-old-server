...
-- ÌØ±ð×¢Òâ,ÕâÀïtoÕ ðµ ¶¨Òå,Í¬ÊÂTÕi pc_xunluoing.lua ÖÐÒ²ÓÐÏàÍ¬toÕ ðµ ¶¨Òå,C¥n Í¬²½¸Ä
x300024_g_MonsterId = {
...
function x300024_OnDefaultEvent( sceneId, selfId, BagIndex )
	...
	--  ðÕt ðßþcµ±Ç°toÕ ðµ Ëæ»úÖµ,¿´¿´Ðúng²»Ðúng0,Èç¹ûÐúng0,¾Í´ÓÐÂÈ¡µ giâyæ»úÊý
	...
	-- ÅÐ¶¨Íæ¼ÒÐúng²»ÐúngÒÑ¾­½øÈëÁËCái này ÇøÓò
	-- È¡ ði¬mÍæ¼Òµ±Ç°×ø±ê
	...
	if Distance > 5  then
		-- Íæ¼Ò ði¬m»÷Cái này ÎïÆ·,»áµ¯³öÑ²ÂßÁîtoÕ ðµ ,¸æËßÍæ¼ÒTÕi Ê²Ã´µØ ði¬m¿ÉÒÔÊ¹ÓÃCái này ÎïÆ·
		BeginEvent(sceneId)
		...
		if DelItem( sceneId, selfId, 40004413, 1 ) > 0 then
			...
			-- ÕâÀï¸ù¾ÝÈÎÎñÖÐ±£´ætoÕ ðµ Êý¾ÝÀ´·Å¹Ö
			...
			for i ,nn in x300024_g_MonsterId  do
				if nn.n == nIndex  then
					...
					-- ÉèÖÃCái này ¹ÖtoÕ ðµ ×Ô¶¯ÏûÊ§Ê±¼ä  (15·ÖÖÐ.)
					...
					-- ÉèÖÃCái này ¹ÖÎª²»ÄÜ¹¥»÷
					local curCampID = 8 --¶ÔÍæ¼Ò¶Ô¹Ö¶¼ÐúngÓÑºÃtoÕ ðµ ÕóÓª
					SetNpcCamp(sceneId, MonsterId, curCampID)

					-- ±£´æCái này ¹ÖÎïtoÕ ðµ ±àºÅ,Í¬Ê±±£´æmµt cáiÒÑ¾­·Å³ö¹ÖÎïtoÕ ðµ ±êÖ¾
					...
					-- °Ñ¹ÖtoÕ ðµ TênÉèÖÃÎª³ÆºÅ,È»ºóTênÓÃCái này ±íÊ¾
