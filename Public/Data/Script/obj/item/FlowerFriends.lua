...
--½Å±¾ºÅ
x889036_g_scriptId = 889036 --ÁÙÊ±Ğ´Cái này ,ÕæÕıÓÃtoÕ ğµ Ê±ºòmµt ¶¨Òª¸Ä.
...
x889036_g_standFlag = 1 -- 2:¶ÓÓÑ, 1: ÓÑ¾ü, -1: µĞ¾ü
--AEÓ°ÏìÊıÄ¿ÏŞÖÆ
x889036_g_effectCount = 4 -- -1:²»ÏŞÖÆ
--Ğ§¹ûtoÕ ğµ ID
x889036_g_Impact1 = 4906 --ÁÙÊ±Ğ´Cái này 
x889036_g_Impact2 = -1 --²»ÓÃ
...
--		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x889036_g_Impact1, 0);
		...
		if(0<=targetId) then
			if LuaFnIsFriend(sceneId, targetId, selfId) > 0 then
				...
				BeginEvent(sceneId)
			    AddText(sceneId, "Ngß½i cùng ğ¯i phß½ng ğµ hæu häo gia tång 100 r°i");
			  EndEvent(sceneId)
