...
x702002_g_HPValue = 5000			-- ÉúÃüÖµÔö¼ÓÖµ
x702002_g_MaxHPValue = 0			-- ×î´óÉúÃüÖµÔö¼ÓÖµ
x702002_g_LifeValue = 0				-- ÊÙÃüÔö¼ÓÖµ
x702002_g_HappinessValue = 1	-- ¿ìÀÖ¶ÈÔö¼ÓÖµ
x702002_g_MaxUseCount = 100		-- ×î´óÊ¹ÓÃ´ÎÊı100´Î
...
function x702002_OnActivateOnce( sceneId, selfId )
  ...
  local ValidValue = x702002_g_MaxUseCount - UseValue
	...
	if valueHP == MaxHP and valueHappy == MaxHappiness then
		x702002_ShowMsg( sceneId, selfId, "Trân thú không c¥n sØ døng thÑc ån")
		return 0
	end
	...
	if bagId >= 0 then		
		...
		if UseValue >= x702002_g_MaxUseCount then   --¼ÇÂ¼toÕ ğµ Ê¹ÓÃ´ÎÊı´óÓÚµÈÓÚ×î´ó¿ÉÓÃ´ÎÊı,ÀíÂÛÉÏ²»¿ÉÄÜ³öÏÖ.
		    return 0
		end
		...
		SetBagItemParam( sceneId, selfId, bagId, 4, 2, x702002_g_MaxUseCount ) --ÉèÖÃ×î´ó´ÎÊı
		...
		if CurValue >= x702002_g_MaxUseCount then  --µ±Ê¹ÓÃ´ÎÊı´ïµ½×î´ó´ÎÊıÊ±,½«É¾³ı´ËÎïÆ·
