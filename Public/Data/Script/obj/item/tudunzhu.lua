...
-- 300056
-- ÎïÆ·¿ÉÒÔÊ¹ÓÃ10´Î
-- µÚmµt ´ÎÊ¹ÓÃ¡°¶¨Î»Tr· v« ği¬m¡±
-- ÒÔºóÃ¿´ÎÊ¹ÓÃ¿Ûmµt ´Î,¿ÛÍê¾ÍÉ¾³ıÎïÆ·,Ë²ÒÆµ½Tr· v« ği¬m
...
-- ÏŞÖÆÊ¹ÓÃ´«ËÍ¹¦ÄÜtoÕ ğµ ³¡¾°
x300056_g_NoChuangsongScn=
{
	151		-- ¼àÓü
}

-- ÏŞÖÆÊ¹ÓÃÍÁ¶İÖé¶¨Î»toÕ ğµ ³¡¾°
...
x300056_g_Impact_NotTransportList = { 5929 } -- ½ûÖ¹´«ËÍtoÕ ğµ Impact
x300056_g_TalkInfo_NotTransportList = { "#{GodFire_Info_062}" } -- ½ûÖ¹´«ËÍtoÕ ğµ ImpactÌáÊ¾ĞÅÏ¢
...
function x300056_OnConditionCheck( sceneId, selfId , idid)
	...
	-- 1,¼ì²âCái này ÎïÆ·Ğúng²»ĞúngÓĞ¼ÇÂ¼toÕ ğµ Êı¾İÁË,
	-- C¥n ¼ÇÂ¼toÕ ğµ Êı¾İĞúng,Ê¹ÓÃ´ÎÊı,¶¨Î»³¡¾°Id,ÒÑ¾­×ø±ê
...
function x300056_CallMe( sceneId, selfId, nItemIndex, PlayerGuid)	
	...
	-- ¼ì²âCái này Íæ¼ÒĞúng²»ĞúngÄÜ¹»Ê¹ÓÃÍÁ¶İÖé¶¨Î»
	...
	-- ¿´PlayerGuid Ğúng²»Ğúng¶ÓÎéÖĞtoÕ ğµ ÈË,È»ºóÔÙ¿´Ê±¼äĞúng²»Ğúng¹ıÆÚ,±ğtoÕ ğµ Ò²Ã»ÓĞÊ²Ã´ÄÜ¼ì²étoÕ ğµ ÁË
...
function x300056_OnActivateOnce( sceneId, selfId )
	...
	--	-- ĞÂtoÕ ğµ ĞŞ¸Ä,TÕi ÕâÀïµ¯³ö¶Ô»°¿òÌáÎÊÍæ¼ÒĞúng×ö´«ËÍ»¹Ğúng¶¨Î».=======
...
function x300056_SetPosition( sceneId, selfId, nItemIndex )
	...
	-- mµt cáiĞÂtoÕ ğµ Ã»ÓĞ¶¨Î»¹ıtoÕ ğµ ÎïÆ·
	...
	BeginEvent(sceneId)
		AddText(sceneId,"Ğµn Th± Châu cüa các hÕ ğã ğ¸nh v¸ thành công.")
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
...
function x300056_PlayerGoto( sceneId, selfId, nItemIndex )
	...
	-- 1,¼ì²âCái này ÎïÆ·Ğúng²»ĞúngÓĞ¼ÇÂ¼toÕ ğµ Êı¾İÁË,
	-- C¥n ¼ÇÂ¼toÕ ğµ Êı¾İĞúng,Ê¹ÓÃ´ÎÊı,¶¨Î»³¡¾°Id,ÒÔ¼°×ø±ê
	local nUseCount	= GetBagItemParam(sceneId, selfId, nItemIndex, 0, 2)
	...
	if ret == 1  then
		--  ğÕt ğßşcÍæ¼Ò¶ÓÎéÖĞTÕi ¸½½ütoÕ ğµ Ğµi viên 
		...
		if nTeamCount > 0  then
			for i=0, nTeamCount-1  do
				...
				if nPlayerId ~= selfId  then
					-- ¸øCái này Íæ¼Ò·¢ËÍmµt cái´«ËÍÑûÇë
