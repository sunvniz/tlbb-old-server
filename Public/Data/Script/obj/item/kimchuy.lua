...
x300253_g_scriptId = 300253 --ÁÙÊ±Ğ´Cái này ,ÕæÕıÓÃtoÕ ğµ Ê±ºòmµt ¶¨Òª¸Ä.
...
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x300253_OnDefaultEvent( sceneId, selfId, bagIndex )
...
--**********************************
function x300253_IsSkillLikeScript( sceneId, selfId)
...
--**********************************
function x300253_CancelImpacts( sceneId, selfId )
...
--**********************************
function x300253_OnConditionCheck( sceneId, selfId )
	--Ğ£ÑéÊ¹ÓÃtoÕ ğµ ÎïÆ·
...
--**********************************
function x300253_OnDeplete( sceneId, selfId )
...
--**********************************
function x300253_OnActivateOnce( sceneId, selfId )
  ...
  if LuaFnDepletingUsedItem(sceneId, selfId) <= 0 then
	...
	LuaFnAddExp( sceneId, selfId, 5000*playerlvl )
	LuaFnSendSpecificImpactToUnit( sceneId, selfId, selfId, selfId, 18, 0 )
	CallScriptFunction( SCENE_SCRIPT_ID, "PlaySoundEffect", sceneId, selfId, 66 )
  --Ëæ»ú²úÉú²»Í¬toÕ ğµ ½ğ±Ò

  local randitem = random(getn(x300253_g_GetItem))

  local BagIndex = TryRecieveItem( sceneId, selfId, x300253_g_GetItem[randitem], QUALITY_MUST_BE_CHANGE )
  if BagIndex ~= -1 then
    --·¢¹«¸æ....
...
--×¢: ÕâÀïĞúng¼¼ÄÜÉúĞ§mµt ´ÎtoÕ ğµ Èë¿Ú
--**********************************
function x300253_OnActivateEachTick( sceneId, selfId)
