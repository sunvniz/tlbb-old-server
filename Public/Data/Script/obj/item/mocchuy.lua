...
x300251_g_scriptId = 300251 --ÁÙÊ±Ğ´Cái này ,ÕæÕıÓÃtoÕ ğµ Ê±ºòmµt ¶¨Òª¸Ä.
...
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x300251_OnDefaultEvent( sceneId, selfId, bagIndex )
...
--**********************************
function x300251_IsSkillLikeScript( sceneId, selfId)
...
--**********************************
function x300251_CancelImpacts( sceneId, selfId )
...
--**********************************
function x300251_OnConditionCheck( sceneId, selfId )
	--Ğ£ÑéÊ¹ÓÃtoÕ ğµ ÎïÆ·
...
--**********************************
function x300251_OnDeplete( sceneId, selfId )
...
--**********************************
function x300251_OnActivateOnce( sceneId, selfId )
  ...
  if LuaFnDepletingUsedItem(sceneId, selfId) <= 0 then
	...
	LuaFnAddExp( sceneId, selfId, 100*playerlvl )
  ...
  local randitem = random(getn(x300251_g_GetItem))

  local BagIndex = TryRecieveItem( sceneId, selfId, x300251_g_GetItem[randitem], QUALITY_MUST_BE_CHANGE )
  if BagIndex ~= -1 then
    --·¢¹«¸æ....
...
--×¢: ÕâÀïĞúng¼¼ÄÜÉúĞ§mµt ´ÎtoÕ ğµ Èë¿Ú
--**********************************
function x300251_OnActivateEachTick( sceneId, selfId)
