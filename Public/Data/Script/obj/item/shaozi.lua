...
function x300001_OnDefaultEvent( sceneId, selfId, BagIndex )
	if ( GetHumanWorldX(sceneId,selfId) >= 225 ) and ( GetHumanWorldX(sceneId,selfId) <= 243 ) then		--X×ø±êThöa mãnÒªÇó
		if (GetHumanWorldZ(sceneId,selfId) >= 12) and (GetHumanWorldZ(sceneId,selfId) <= 28 ) then		--Z×ø±êThöa mãnÒªÇó
			if LuaFnGetCopySceneData_Param(sceneId, 2) == 0 then			--³¡¾°±äÁ¿Îª0,³¡¾°±äÁ¿TÕi ·þÎñÆ÷Æô¶¯Ê±Îª0,Ö®ºóÓÉ½Å±¾¿ØÖÆ
				npcobjid = LuaFnCreateMonster( sceneId, 28, 234,18, 3,2, 019000 ) ;
				LuaFnSetCopySceneData_Param(sceneId, 2, npcobjid);
				--ÉèÖÃ¶¨Ê±Æ÷,ÓÃÀ´¿ØÖÆ20 giâyºóÂ³Æ½ÏûÊ§
