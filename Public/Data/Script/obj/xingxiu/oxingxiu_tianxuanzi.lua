...
function x016035_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		AddNumText(sceneId,x016035_g_scriptId,"Ðánh nh®n ð¬ tu luy®n",10,0)
	EndEvent(sceneId)
...
function x016035_OnEventRequest( sceneId, selfId, targetId, eventId )
	if	GetNumText()==0	then
		if	GetLevel( sceneId, selfId)<51  then	
			BeginEvent( sceneId )
			local strText = "Ngß¶i trong giang h° bß¾c vào tu luy®n, ph¥n l¾n ð«u t× c¤p 51 tr· lên, th¤y võ công cüa ngß½i cûng t¥m thß¶ng, ta không d¡t ngß½i ði ðâu"
