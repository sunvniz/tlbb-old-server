...
--»î¶¯±àºÅ,Õâ cáiÊı×ÖC¥n ºÍ»î¶¯±íÖĞtoÕ ğµ ¶ÔÓ¦
--ÆäÖĞtoÕ ğµ Êı¾İtoÕ ğµ µÚ0Î»Î»ÖÃÓÃÀ´±íÊ¾»î¶¯Ğúng·ñM· ra 
x001230_g_nActivityId = 9

-- Õâ cái»î¶¯ÆäÊµ²»C¥n ´´½¨¸±±¾,Ö±½Ó°ÑËùÓĞtoÕ ğµ ÈË,·ÅÈëmµt  cái³¡¾°¾Í¿ÉÒÔÁË 
x001230_g_nScnenId = 125

-- ÏÖTÕi Ğúng²»Ğúng»î¶¯¼ÆÊıÊ±¼ätoÕ ğµ ±êÖ¾
x001230_g_bHuashaning = 0

-- 9 cáiÃÅÅÉtoÕ ğµ É±ÈËÊı
...
-- 9 cáiÃÅÅÉÇ°ÈıÃûtoÕ ğµ Íæ¼ÒID		
-- ²ÎÊı take ±íÊ¾¸ÃÍæ¼ÒĞúng·ñÁì¹ı½±Àø
...
x001230_g_PlayerKc = {}				-- Ã¿ cáiÍæ¼ÒÉ±ÈËtoÕ ğµ ÊıÄ¿
x001230_g_PlayerName = {}			-- Ã¿ cáiÍæ¼ÒtoÕ ğµ Tên
x001230_g_PlayerNum = 0				-- Íæ¼ÒÊıÄ¿

--[ QUFEI 2007-07-27 15:04 UPDATE ]
x001230_g_PlayerMenpai = {}		-- Ã¿ cáiÍæ¼ÒtoÕ ğµ ÃÅÅÉ
x001230_g_PlayerId = {}

x001230_g_PreTime = 0


--³¡¾°ÖĞÑëtoÕ ğµ 2 cáiBí T¸chË¢ĞÂ ği¬m....
x001230_g_MJPosA = {
	{x=135, y=125},{x=141, y=123}
}

--³¡¾°ËÄÖÜtoÕ ğµ 6 cáiBí T¸chË¢ĞÂ ği¬m....
x001230_g_MJPosB = {
	{x=143, y=140, rand=-1},
	{x=153, y=121, rand=-1},
	{x=150, y=110, rand=-1},
...
function x001230_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x001230_g_Update["id01"] then
        ...
        -- 0,´¦ÓÚË«ÈËÆï³Ë×´Ì¬toÕ ğµ ÈË,²»ÄÜ±¨Ãû
        ...
        -- 2,¼ì²â»î¶¯Ğúng²»ĞúngÒÑ¾­¿ªÊ¼ÁË,Èç¹ûÒÑ¾­¿ªÊ¼,¾Í²»ÄÜÔÙ½øÈ¥ÁË
        ...
        -- 3,Ê±¼ä¼ì²â,Tiªp thø±¨ÃûtoÕ ğµ Ê±¼ä,8 ği¬mµ½8 ği¬m30·Ö
        --begin modified by zhangguoxin 090207
        --local nHour = GetHourTime()
        --local temp = floor(nHour/100)*100
        --PrintNum(nHour-temp)
        --if ((nWeek==0) and (nHour-temp<60 or nHour-temp>=62)) or 
        --     ( (nWeek==2 or nWeek==4) and (nHour-temp<76 or nHour-temp>=78))  then		--±¨ÃûÊ±¼ä2 4toÕ ğµ 19:00-19:30,ÈÕtoÕ ğµ 15:00-15:30
        local nQuarter = mod(GetQuarterTime(),100);
        if ((nWeek==0) and (nQuarter < 60 or nQuarter >= 62 )) or 
             ( (nWeek==2 or nWeek==4) and (nQuarter < 76 or nQuarter >= 78))  then		--±¨ÃûÊ±¼ä2 4toÕ ğµ 19:00-19:30,ÈÕtoÕ ğµ 15:00-15:30
        ...
        -- 4,ÈËÊı¼ì²â,Ê¹ÓÃ»î¶¯Êı¾İÇøtoÕ ğµ Êı¾İÀ´±£´æÕâĞ©Êı¾İ
        ...
        -- a,¼ì²âÍæ¼ÒËùTÕi ÃÅÅÉĞúng²»ĞúngÒÑ¾­ÓĞ10ÈËÁË,
        if nPeopleNum < 10 then
            -- Ëû¿ÉÒÔ½øÈë
        elseif nPeopleNum >=10 and nPeopleNum < 30  then
            -- ÅĞ¶ÏĞúng²»ĞúngÈËÊı×î¶àtoÕ ğµ ÃÅÅÉ,
            for i=0, 8   do
                if nMenpai ~= i   then
            ...
            -- C¥n µÈ´ı±ğtoÕ ğµ ÃÅÅÉ£«ÈËºó½øÈë
            BeginEvent(sceneId)
                AddText(sceneId,"#BHoa S½n lu§n kiªm");
        ...
        -- ÆäËûtoÕ ğµ mµt Ğ©¿ÉÒÔ½øÈëHoa S½n lu§n kiªmtoÕ ğµ ÅĞ¶¨
        -- PrintNum(selfId)
        -- local Name = GetName(sceneId, selfId)
        -- LuaFnSendSystemMail( sceneId, Name, "mail test" )
        -- ×¢Òâ,Õâ cáiH® th¯ngĞúng²»´´½¨¸±±¾toÕ ğµ ,Ö±½Ó½øÈë³¡¾°¾Í¿ÉÒÔÁË
        local nPos_X;
        local nPos_Z;
   ...
   elseif key == x001230_g_Update["id02"] then
        -- Hoa S½n lu§n kiªm½±Àø ÖÜÈÕ: 16:00-17:00 ÖÜ2¡¢ÖÜ4: 20:00-21:00
        local nWeek = GetTodayWeek()
        --begin modified by zhangguoxin 090207
				--local nHour = GetHourTime()
				--local temp = floor(nHour/100)*100
...
function x001230_OnEnumerate( sceneId, selfId, targetId )	
    ...
    --Hoa S½n lu§n kiªm½áThúc Hàmµt Ğ¡Ê±ÄÚLînh ½±Àø	ÖÜÈÕ: 16:00-17:00 ÖÜ2¡¢ÖÜ4: 20:00-21:00
    local nWeek = GetTodayWeek()
		 				...
		 				if index01 == 0 and index02 == 0 and index03 == 1 then						 				
								...
								if ret1 > 0 and ret2 > 0 then
										...
										BroadMsgByChatPipe(sceneId,selfId, strText, 7)				 																			
						 				...
						 				BeginEvent(sceneId)
										strText = "Cµt v§t ph¦m ho£c cµt nguyên li®u không ğü ch± tr¯ng, vui long s¡p xªp lÕi r°i hãy ğªn nh§n lînh."
										AddText(sceneId,strText);
										EndEvent(sceneId)
										DispatchMissionTips(sceneId,selfId)
								end			 				
...
function x001230_OnHuashanSceneTimer( sceneId, selfId )
	...
	if x001230_g_bEndTime ~= 0 and nNowTimeEX > x001230_g_bEndTime+60    then
		...
		BroadMsgByChatPipe(sceneId,0,"@*;SrvMsg;SCA:" .. "#cff99cc Trong l¥n Lu§n Kiªm Hoa S½n này, #Y" .. x001230_g_bMenpai .. "#cff99cc Träi qua nhæng n± lñc, cu¯i cùng ğã ğÕt ğßşc vinh quang,#Y Lu§n Kiªm Hoa S½n hÕng nh¤t#cff99cc, ğ¬ khích l®, t¤t cä ğ® tØ môn hÕ m²i ngày s¨ ğßşc t£ng thß·ng #Yg¤p 3 l¥n kinh nhi®m#W khi làm vòng nhi®m vø sß môn ğ¥u tiên cho ğªn kÏ Lu§n Kiªm Hoa S½n l¥n sau kªt thúc.",4)
	...
	-- ¼ì²âÕâ cái³¡¾°ÄÚĞúng²»ĞúngÓĞÍæ¼Ò,Èç¹ûÃ»ÓĞ,Ö±½ÓTr· v«
	local nHumanNum = LuaFnGetCopyScene_HumanCount(sceneId)
	if nHumanNum == 0  then
		return
	end
	
	--¼ì²âµ±Ç°Ğúng²»ĞúngHoa S½n lu§n kiªmtoÕ ğµ »î¶¯Ê±¼ä,Èç¹û²»Ğúng,¾Í°Ñ³¡¾°ÄÚtoÕ ğµ ËùÓĞÍæ¼Ò¶¼T³öÈ¥
	local bIsTime = 1
	...
	--  ÕıÊ½»î¶¯Ê±¼ä ÖÜÈÕ: 15:00-16:00 ÖÜ2¡¢ÖÜ4: 19:00-20:00
	...
	if bIsTime == 1   then
		if x001230_g_bHuashaning == 0   then
			...
			-- ¿ªĞúngmµt ´ÎĞÂtoÕ ğµ »î¶¯Ç°,C¥n ÇåÀíÉ±ÈË¼ÆÊıÆ÷
		...
		if x001230_g_bHuashaning == 1   then
			...
			if nNowTime > x001230_g_PreTime+300    then
				...
				local szStr = "#{HSJL_090113_01}" --"»ªÉ½Ö®µß,ÌìÏÂÃÅÅÉ¸÷³ÊĞÛ·ç,Ä¿Ç°´ÓÊµÁ¦ÉÏÅÅÃûTÕi µÚmµt µÚ¶şÓëµÚÈıÃûtoÕ ğµ ÃÅÅÉ: ·Ö±ğÎª" zchw
	...
	-- Èç¹û bIsTime == 0 ,¾Í°ÑËùÓĞtoÕ ğµ Íæ¼ÒT³öÈ¥
	if bIsTime == 0   then
		
		if x001230_g_bHuashaning == 1   then
			-- »î¶¯½áÊøtoÕ ğµ Ê±¿Ì,ÕâĞúngÊ±ºòÒª´¦Àí±È½Ï¶à¶«Î÷,×¢Òâ£..¡
			-- 1,¿´ÏÂË­Ğúng½ñÌìtoÕ ğµ Ê¤ÀûÕß,Èç¹ûÓĞ¶à cái²¢ÁĞµÚmµt ,¾Í¡°Ëæ»ú¡±³ömµt  cáiÊ¤ÀûÕß
			--  a,ÏÖÅÅĞòËùÓĞtoÕ ğµ É±ÈËÊı
				x001230_g_bHuashaning = 0
				...
				-- TÕi  ği¬mµ½toÕ ğµ É±ÈËÊı1toÕ ğµ ÃÅÅÉÖĞÈ¥Ëæ»úmµt  cái,ºÇºÇ,´«ËµÖĞtoÕ ğµ Ç¿X
				local nVMenpai = random(nMax)

				if x001230_g_nMenpaikills[nVMenpai].kill < 1  then
					-- ±¾´Î»î¶¯ÎŞĞ§,¶¼Ã»ÓĞÉ±ÈË,ÄÄÀïÀ´toÕ ğµ Ê¤ÀûÕß
					return
				end
				
				-- ÉèÖÃÈ«¾ÖtoÕ ğµ Êı¾İ,Ê¤Àû×ÅÃÅÅÉ,×¢Òâ,ÕâÀï+ĞúngÌØ±ğC¥n toÕ ğµ ,ÒòÎª0Ğúngmµt  cái³õÊ¼Öµ
				...
				--AddGlobalCountNews( sceneId, "#PTÕi ±¾½ìHoa S½n lu§n kiªmµ±ÖĞ,#W" .. szMenpai .. "#P¾­¹ıÉÏÏÂmµt ĞÄtoÕ ğµ Å¬Á¦,ÖÕÓÚ ğÕt ğßşcHoa S½n lu§n kiªmµÚmµt toÕ ğµ ÈÙÓş,×÷Îª½±Àø,ÆäÃÅÏÂËùÓĞµÜ×ÓTÕi ÏÂ½ìHoa S½n lu§n kiªm»î¶¯½áÊøÇ°,Ã¿ÌìtoÕ ğµ µÚmµt ÂÖNhi®m vø sß môn½«»á±äÎª#Y3±¶#PKinh nghi®m." )
				
				x001230_g_bMenpai = szMenpai
				-- BroadMsgByChatPipe(sceneId,0,"@*;SrvMsg;SCA:" .. "#PTÕi ±¾½ìHoa S½n lu§n kiªmµ±ÖĞ,#Y" .. szMenpai .. "#P¾­¹ıÉÏÏÂmµt ĞÄtoÕ ğµ Å¬Á¦,ÖÕÓÚ ğÕt ğßşcHoa S½n lu§n kiªmµÚmµt toÕ ğµ ÈÙÓş,×÷Îª½±Àø,ÆäÃÅÏÂËùÓĞµÜ×ÓTÕi ÏÂ½ìHoa S½n lu§n kiªm»î¶¯½áÊøÇ°,Ã¿ÌìtoÕ ğµ µÚmµt ÂÖNhi®m vø sß môn½«»á±äÎª#Y3±¶Kinh nghi®m.",4)
				...
				--  ğÕt ğßşcÃÅÅÉÇ°ÈıÃûtoÕ ğµ Íæ¼ÒTên
				local num = {}
				for i=0 ,8  do
					num[i] = 0
				end
          ...
          AuditHuashanThreeWinners(sceneId,x001230_g_nMenpaiQiansan[i].playerid03)
				...
				for i=1, x001230_g_PlayerNum  do
					...
					if mingci == 1 and x001230_g_PlayerName[i] ~= "" then
						...
						mailStr = "Chúc m×ngÄú ğÕt ğßşc±¾½ìHoa S½n lu§n kiªm×Ü»ı·ÖµÚmµt Ãû!Äú¿ÉÒÔÕÒHoa S½n lu§n kiªmÖ÷³ÖÈËËÕ½£Áë(Tô Châu193,138)Lînh ·áºñ½±Àø!×¢Òâ: Lînh Ê±¼ä1 cáiĞ¡Ê±,Èô³¬¹ı1 cáiĞ¡Ê±ÈÔÎ´Lînh ½±Àø¾ÍÖ»ÄÜÏÂ´ÎÔÙ¼ÓÓÍ¿©!Lînh ½±ÀøÖ®Ç°ÇëÏÈ ği¬m»÷È·ÈÏ½±ÀøÑ¡Ïî,ÊÕÈ¡Áì½±ÓÊ¼şºó, ği¬m»÷Lînh Hoa S½n lu§n kiªm½±Àø·½¿ÉÁì½±³É¹¦."
					...
					elseif mingci == 2 and x001230_g_PlayerName[i] ~= "" then
						...
						mailStr = "Chúc m×ngÄú ğÕt ğßşc±¾½ìHoa S½n lu§n kiªm×Ü»ı·ÖµÚ¶şÃû!Äú¿ÉÒÔÕÒHoa S½n lu§n kiªmÖ÷³ÖÈËËÕ½£Áë(Tô Châu193,138)Lînh ·áºñ½±Àø!×¢Òâ: Lînh Ê±¼ä1 cáiĞ¡Ê±,Èô³¬¹ı1 cáiĞ¡Ê±ÈÔÎ´Lînh ½±Àø¾ÍÖ»ÄÜÏÂ´ÎÔÙ¼ÓÓÍ¿©!Lînh ½±ÀøÖ®Ç°ÇëÏÈ ği¬m»÷È·ÈÏ½±ÀøÑ¡Ïî,ÊÕÈ¡Áì½±ÓÊ¼şºó, ği¬m»÷Lînh Hoa S½n lu§n kiªm½±Àø·½¿ÉÁì½±³É¹¦."
					...
					elseif mingci == 3 and x001230_g_PlayerName[i] ~= "" then
						...
						mailStr = "Chúc m×ngÄú ğÕt ğßşc±¾½ìHoa S½n lu§n kiªm×Ü»ı·ÖµÚÈıÃû!Äú¿ÉÒÔÕÒHoa S½n lu§n kiªmÖ÷³ÖÈËËÕ½£Áë(Tô Châu193,138)Lînh ·áºñ½±Àø!×¢Òâ: Lînh Ê±¼ä1 cáiĞ¡Ê±,Èô³¬¹ı1 cáiĞ¡Ê±ÈÔÎ´Lînh ½±Àø¾ÍÖ»ÄÜÏÂ´ÎÔÙ¼ÓÓÍ¿©!Lînh ½±ÀøÖ®Ç°ÇëÏÈ ği¬m»÷È·ÈÏ½±ÀøÑ¡Ïî,ÊÕÈ¡Áì½±ÓÊ¼şºó, ği¬m»÷Lînh Hoa S½n lu§n kiªm½±Àø·½¿ÉÁì½±³É¹¦."
					...
					elseif mingci == 4  then
						-- str = "µÚËÄÃû: " .. x001230_g_PlayerName[i]
						mingci = 5
						if x001230_g_PlayerKc[i] ~= x001230_g_PlayerKc[i+1]  then
							mingci = 5
						end
					...
					elseif mingci == 5  then
						-- str = "µÚÎåÃû: " .. x001230_g_PlayerName[i]
						mingci = 6
						if x001230_g_PlayerKc[i] ~= x001230_g_PlayerKc[i+1]  then
							mingci = 6
							break
				...
				for i=1, 9  do
					--ÏòÃ¿ cáiÃÅÅÉ·¢ËÍÇ°ÈıÃûtoÕ ğµ ÃÅÅÉ¹«¸æ
					str = "#cff99ccLu§n Kiªm Hoa S½n l¥n này ğã kªt thúc, các ğ°ng môn trß¾c sau ğã d¯c sÑc vì môn phái, ğã th¬ hi®n sñ dûng cäm trong chiªn ğ¤u, trong s¯ h÷ ngß¶i ğßşc xªp cao nh¤t là: #W"
					...
					if x001230_g_nMenpaiQiansan[i].player01 ~= ""
						...
						mailStr = "Chúc m×ngÄú ğÕt ğßşc±¾½ìHoa S½n lu§n kiªm±¾ÃÅÅÉµÚmµt Ãû!Äú¿ÉÒÔÕÒHoa S½n lu§n kiªmÖ÷³ÖÈËËÕ½£Áë(Tô Châu193,138)Lînh ·áºñ½±Àø!×¢Òâ: Lînh Ê±¼ä1 cáiĞ¡Ê±,Èô³¬¹ı1 cáiĞ¡Ê±ÈÔÎ´Lînh ½±Àø¾ÍÖ»ÄÜÏÂ´ÎÔÙ¼ÓÓÍ¿©!Lînh ½±ÀøÖ®Ç°ÇëÏÈ ği¬m»÷È·ÈÏ½±ÀøÑ¡Ïî,ÊÕÈ¡Áì½±ÓÊ¼şºó, ği¬m»÷Lînh Hoa S½n lu§n kiªm½±Àø·½¿ÉÁì½±³É¹¦."
					...
					if x001230_g_nMenpaiQiansan[i].player02 ~= ""
						...
						mailStr = "Chúc m×ngÄú ğÕt ğßşc±¾½ìHoa S½n lu§n kiªm±¾ÃÅÅÉµÚ¶şÃû!Äú¿ÉÒÔÕÒHoa S½n lu§n kiªmÖ÷³ÖÈËËÕ½£Áë(Tô Châu193,138)Lînh ·áºñ½±Àø!×¢Òâ: Lînh Ê±¼ä1 cáiĞ¡Ê±,Èô³¬¹ı1 cáiĞ¡Ê±ÈÔÎ´Lînh ½±Àø¾ÍÖ»ÄÜÏÂ´ÎÔÙ¼ÓÓÍ¿©!Lînh ½±ÀøÖ®Ç°ÇëÏÈ ği¬m»÷È·ÈÏ½±ÀøÑ¡Ïî,ÊÕÈ¡Áì½±ÓÊ¼şºó, ği¬m»÷Lînh Hoa S½n lu§n kiªm½±Àø·½¿ÉÁì½±³É¹¦."
					...
					if x001230_g_nMenpaiQiansan[i].player03 ~= ""
						...
						mailStr = "Chúc m×ngÄú ğÕt ğßşc±¾½ìHoa S½n lu§n kiªm±¾ÃÅÅÉµÚÈıÃû!Äú¿ÉÒÔÕÒHoa S½n lu§n kiªmÖ÷³ÖÈËËÕ½£Áë(Tô Châu193,138)Lînh ·áºñ½±Àø!×¢Òâ: Lînh Ê±¼ä1 cáiĞ¡Ê±,Èô³¬¹ı1 cáiĞ¡Ê±ÈÔÎ´Lînh ½±Àø¾ÍÖ»ÄÜÏÂ´ÎÔÙ¼ÓÓÍ¿©!Lînh ½±ÀøÖ®Ç°ÇëÏÈ ği¬m»÷È·ÈÏ½±ÀøÑ¡Ïî,ÊÕÈ¡Áì½±ÓÊ¼şºó, ği¬m»÷Lînh Hoa S½n lu§n kiªm½±Àø·½¿ÉÁì½±³É¹¦."
			...
			if LuaFnIsObjValid( sceneId, nHumanId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, nHumanId ) == 1 then
			  ...
			  DispatchMissionTips(sceneId, nHumanId)
				...
				elseif nMempai == 9    then
					--Ã»ÓĞÃÅÅÉtoÕ ğµ ÈËÓ¦¸ÃĞúng²»³öÏÖtoÕ ğµ ,±ÜÃâ´íÎó³öÏÖ,ÉèÖÃ cáiÎ»ÖÃ
					nMenpaiScene = 1
					nPos_X			 = 191
					nPos_Z			 = 138
					--PrintStr("ÄãÕâ cái¼mµt ï,±»ÈËKtoÕ ğµ ¶¼²»ÖªµÀ×Ô¼ºÃÅÅÉÁË???")
				end
...
function x001230_KillPlayer(sceneId, killerId, diedId)
	-- Ö»ÓĞTÕi »î¶¯Ê±¼äÄÚÉ±toÕ ğµ ÈË²ÅĞúngÓĞĞ§toÕ ğµ 
	if x001230_g_bHuashaning == 0  then
		return
	end
	...
	if IsAddCnt == 0 then
		...
		BeginEvent(sceneId)
			AddText(sceneId, "ÓÉÓÚÄúÖØ¸´É±ËÀÍ¬mµt  cáiÍæ¼mµt ò¸ÃÍæ¼ÒÓëÄúµÈc¤pÏà²î³¬¹ı40c¤p,Äú½«²»»á ğÕt ğßşcÃÅÅÉ¹±Ï×¶ÈºÍ»ı·Ö½±Àø.")
		EndEvent(sceneId)
	...
	-- ¸øËùÓĞ³¡¾°ÖĞÉ±ÊÖÃÅÅÉtoÕ ğµ ÈË·¢ÏûÏ¢,¸æËßËûÃÇ ği¬m·ÖÁË
	--1, ğÕt ğßşc³¡¾°ÖĞtoÕ ğµ ËùÓĞÈË,
	local nHumanNum = LuaFnGetCopyScene_HumanCount(sceneId)
...
function x001230_GiveMJ( sceneId )
	...
	for i=1, getn(x001230_g_MJPosB) do
		...
		--Ñ¡³öÈ¨Öµ×î´ótoÕ ğµ 4 cái ği¬m×÷ÎªÍ¶·ÅBí T¸chtoÕ ğµ  ği¬m....
	for i=1, 4 do
