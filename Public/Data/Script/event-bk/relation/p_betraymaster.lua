...
function x806006_OnDefaultEvent( sceneId, selfId, targetId )
	...
	elseif key == 1 then	--Xác nh§nÓëÊ¦¸µ½â³ı¹ØÏµ
		if x806006_CheckAccept( sceneId, selfId, targetId ) > 0 then
			x806006_OnAccept( sceneId, selfId, targetId )
		end
	elseif key == 2 then	--HuÖ böÓëÊ¦¸µ½â³ı¹ØÏµ--add by xindefeng
		--¹Ø±Õ½çÃæ
...
function x806006_OnAccept( sceneId, selfId, targetId )
	...
	--LuaFnSendNormalMail( sceneId, selfId, MasterName, "Ğ® tØ cüa các hÕ " .. selfName .. " không c¥n sñ dÕy d² cüa các hÕ næa nên ğã thoát ly quan h® sß ğ°." )
