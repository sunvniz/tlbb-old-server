...
x806001_g_msg_swear["wait"]	= " µÈ´ýÆäËûÈË»Ø¸´."

--Key for AddNumText
x806001_g_key					= {}
x806001_g_key["swear"]	= 10000	--½á°ÝÁ÷³Ì
x806001_g_key["allow"]	= 10001	--Xác nh§n½á°Ý»¨·Ñ½çÃæ
x806001_g_key["unall"]	= 10002	--HuÖ bö½á°Ý»¨·Ñ½çÃæ
x806001_g_key["confi"]	= 10003	--Xác nh§n½á°ÝÍ¬Òâ½çÃæ
x806001_g_key["uncon"]	= 10004	--HuÖ bö½á°ÝÍ¬Òâ½çÃæ
x806001_g_key["title"]	= 20000	--Lînh ³ÆºÅ
...
function x806001_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x806001_g_key["confi"] then
		...
		--²é¿´Ðúng·ñËùÓÐtoÕ ðµ Ðµi viên ¶¼Í¬ÒâÁË
...
function x806001_CheckAccept( sceneId, selfId, targetId )
	...
	--(4)Ðµi viên ÖÐÐúng·ñ´æTÕi ·òÆÞ¡¢Ê¦Í½¡¢½á°Ý¡¢·ÇºÃÓÑ¹ØÏµ
	local i = 0
	local j = 0
	local theID = 0
	local otherID = 0
	...
	local Friend2 = 0
	local Brothers = 0
	local BrotherNum = LuaFnIsSweared(sceneId, selfId) --¶Ó³¤Ðúng·ñ½á°Ý¹ý
	while i < TeamSizeSameScene do
		...
		if theSwear > 0 then --Ðµi viên ÓÐ½á°Ý¹ØÏµ
		...
		--Èç¹û¶Ó³¤ÒÑ¾­ÓÐ½á°Ý¹ØÏµ,½«¶ÓÎéÖÐËùÓÐtoÕ ðµ Óë¶Ó³¤Ðúng huynh ð®toÕ ðµ ÈË·Åµ½DICÀïÃæÈ¥
	...
	--(5)Èç¹û¶Ó³¤ÓÐ½á°Ý¹ØÏµ,¿´¿´ËùÓÐtoÕ ðµ  huynh ð®Ðúng·ñ¶¼TÕi Cái này ¶ÓÎéÀïÃæ
	if BrotherNum > 0 then
		--  huynh ð®ÊýµÈÓÚÍ¬³¡¾°toÕ ðµ Ðµi viên Êý
...
function x806001_CheckDrawTitle( sceneId, selfId, targetId )
	...
	--(4)Ðúng·ñ¶ÓÎéÖÐÓÐ²»Ðúng½á°Ý huynh ð®toÕ ðµ Ðµi viên 
	local i = 0
	local theID = 0
	local Brothers = 0
	while i < TeamSizeSameScene do
	...
	--(5)Ðúng·ñËùÓÐÐµi viên ¶¼À´ÁË
	if BrotherNum ~= Brothers then
		x806001_MessageBox( sceneId, selfId, targetId, x806001_g_msg_swear["caa"] )
		return 0
	end

	--(6)Ðúng·ñÓÐ×ã¹»toÕ ðµ ½ðÇ®
	--hzp 2008-12-26
		local nMoneyJZ = GetMoneyJZ(sceneId, selfId);
		local nMoneyJB = LuaFnGetMoney(sceneId, selfId);
		local nMoneySelf = nMoneyJZ + nMoneyJB;
	...
	--(7)Ðúng·ñÒÑ¾­Áì¹ý³ÆºÅÁË
	--Lînh ¹ý³ÆºÅºó,ÈÔÈ»¿ÉÒÔLînh ³ÆºÅ,ÓÃÓÚµ±½á°ÝÈËÊý·¢Éú±ä»¯Ê±,ÐÞ¸Ä½á°Ý huynh ð®toÕ ðµ ³ÆºÅ
...
--**********************************
--µÈ´ýÆäËûÈËXác nh§ntoÕ ðµ ¶Ô»°´°¿ÚÐÅÏ¢ÌáÊ¾
--**********************************
function x806001_WaitSwear( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		AddText( sceneId, x806001_g_msg_swear["wait"] )		
		AddNumText( sceneId, x806001_g_ScriptId, "ÎÒ²»Ïë½á°ÝÁË", 8, x806001_g_key["uncon"] )
	EndEvent( sceneId )
	DispatchEventList( sceneId, selfId, targetId )
end
