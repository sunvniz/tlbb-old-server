...
x806008_g_msg["ts"]	= "  ÒÑ¾­ÐúngÊ¦Í½¹ØÏµÁË,²»ÄÜ°ÝÊ¦."
...
function x806008_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == -1 then
		...
		--BeginEvent( sceneId )
		--	AddText( sceneId, "ÄãÐúng·ñÔ¸Òâ°Ý"..MasterName.."ÎªÊ¦?" )
		--EndEvent( sceneId )
...
function x806008_CheckAccept( sceneId, selfId, targetId )
	...
	--06.10.24,×îÐÂtoÕ ðµ ²ß»®ÎÄµµÒÑ¾­°Ñ´Ë²¿·ÖÄÚÈÝÈ¥µôÁË
	--(7)Ðúng·ñÇ¿ÐÐ½â³ýÊ¦Í½¹ØÏµ²»Âú3Ìì
	--if LuaFnGetPrenticeBetrayTime( sceneId, selfId ) < (86400 * 3) then
	--	x806008_MsgBox( sceneId, selfId, targetId, x806008_g_msg["rec_3"] )
	--	return 0
	--end

	--(8)¿´¿´µ±Ç°Í½µÜÊýÐúng·ñÒÑ¾­×î´óÁË
	--¼ÆËã¿ÉÒÔÊÕµ½toÕ ðµ Í½µÜÊý
	local PrenticeNum = 2
