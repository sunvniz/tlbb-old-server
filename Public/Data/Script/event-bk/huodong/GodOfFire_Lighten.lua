...
--Rùa-Thö thi chÕy´«µÝ»î¶¯....

--¿×Ã÷µÆË¢ÐÂ½Å±¾....
--TÕi Rùa-Thö thi chÕy»î¶¯ÖÐÃ¿°ë phútË¢ÐÂmµt ´Î¿×Ã÷µÆ
...
--  ði¬mÁÁ¿×Ã÷µÆËù¸øtoÕ ðµ Kinh nghi®m
...
--C¥n toÕ ðµ Ð§¹ûID	»ð°ÑÊ¹Õß
x808083_g_ImpactID 	= 5929

--¿×Ã÷µÆ»ØÊÕÌØÐ§
x808083_g_ImpactID_LightOver = { 44, 45, 46 }

--Ð§¹ûtoÕ ðµ ID	Èç¹ûÍæ¼ÒÉíÉÏÓÐ SelfimpactID,ÔòÊ¹ÓÃ impactId01,·ñÔòÊ¹ÓÃ impactId02
x808083_g_itemList = {}
...
x808083_g_itemList[3] = { SelfimpactID=5933, impactId01=0,		impactId02=5937 }			-- ²¶ÊÞ¼Ð
...
-- Nhi®m vø Rùa-Thö thi chÕyID
x808083_g_MissionId =1000
x808083_g_MissItemId = 40004444				-- ÈÎÎñÎïÆ·
--ÈÎÎñÐúng·ñÍê³É
x808083_g_Mission_IsComplete = 0			--ÈÎÎñ²ÎÊýtoÕ ðµ µÚ0Î»
-- ÈÎÎñÎïÆ·²ÎÊý±ê¼Ç
x808083_g_LuoYangLighten_Cnt = 5			--¿×Ã÷µÆLÕc Dß½ngM· ra ÊýÁ¿Ë÷Òý
x808083_g_SuZhouLighten_Cnt	 = 7			--¿×Ã÷µÆTô ChâuM· ra ÊýÁ¿Ë÷Òý
x808083_g_DaLiLighten_Cnt		 = 9			--¿×Ã÷µÆÐÕi LýM· ra ÊýÁ¿Ë÷Òý

--Rùa-Thö thi chÕyLÕc Dß½ng´ò¿¨±ê¼Ç
x808083_g_LuoYang_RecordIdx  = 1			--ÈÎÎñ²ÎÊýtoÕ ðµ µÚ1Î»
--Rùa-Thö thi chÕyTô Châu´ò¿¨±ê¼Ç
x808083_g_SuZhou_RecordIdx 	 = 2			--ÈÎÎñ²ÎÊýtoÕ ðµ µÚ2Î»
--Rùa-Thö thi chÕyÐÕi Lý´ò¿¨±ê¼Ç
x808083_g_DaLi_RecordIdx 		 = 3			--ÈÎÎñ²ÎÊýtoÕ ðµ µÚ3Î»
--¿×Ã÷µÆÍê³ÉÊýÁ¿±ê¼Ç
x808083_g_KongMing_Lighten	 = 4			--ÈÎÎñ²ÎÊýtoÕ ðµ µÚ4Î»
--Nhi®m vø Rùa-Thö thi chÕyÃ¿ÌìÂÖÊý±ê¼Ç
x808083_g_RoundIndex 				 = 7			--ÈÎÎñ²ÎÊýtoÕ ðµ µÚ7Î»

-- Rùa-Thö thi chÕy»î¶¯C¥n  ði¬mÈ¼toÕ ðµ ¿×Ã÷µÆ×ÜÊý
x808083_g_KongMing_CntMax		 = 24

-- Ã¿ÂÖ»î¶¯Ã¿cái³¡¾°C¥n Ë¢³ötoÕ ðµ ¿×Ã÷µÆÊýÁ¿
x808083_g_KongMingLight_CntMax	=	{1, 1, 1, 1, 1, 1}

-- Ã¿cái³¡¾°Ã¿´ÎË¢ÐÂtoÕ ðµ ¿×Ã÷µÆÊýÁ¿
x808083_g_KongMingLight_UpdateCnt = 10

-- ¿×Ã÷µÆÖÐtoÕ ðµ ÈÎÎñÎïÆ·ID
...
function x808083_DropKongMingLight( sceneId )
	...
	local nLightCnt = 0
	local nLightType = 782										-- ¿×Ã÷µÆÀàÐÍ
	if sceneId == 6 then
		nLightType = 782
		nLightPosTbl = x808083_g_PosTable_6			-- ÎÞÁ¿É½
	elseif sceneId == 5 then
		nLightType = 782
		nLightPosTbl = x808083_g_PosTable_5			-- Kính K°
	elseif sceneId == 4 then
		nLightType = 783
		nLightPosTbl = x808083_g_PosTable_4			-- Thái H°
	elseif sceneId == 3 then
		nLightType = 783
		nLightPosTbl = x808083_g_PosTable_3			-- Tung S½n
	elseif sceneId == 7 then
		nLightType = 784
		nLightPosTbl = x808083_g_PosTable_7			-- Kiªm Các
	elseif sceneId == 8 then
		nLightType = 784
		nLightPosTbl = x808083_g_PosTable_8			-- Ðôn Hoàng
	else
		return 0
	end
		...
		for i, nItemList in x808083_g_BonusItem do
			if nRandOdds <= nItemList.Odds1 then
				...
				elseif nItemList.ItemID == x808083_g_BonusItem[8].ItemID then
					...
					--È¡ ði¬m±»¹¥»÷ÕßtoÕ ðµ µ±Ç°×ø±ê	
					local PlayerX = GetHumanWorldX(sceneId,selfId)
					local PlayerY = GetHumanWorldZ(sceneId,selfId)
					
					-- ±éÀú³¡¾°ÖÐËùÓÐÍæ¼Ò,²¢¸øÓë±»¹¥»÷Õß¹¥»÷·¶Î§ÄÚtoÕ ðµ Íæ¼Ò¼ÓImpact
					...
					for j=0, nPlayerCnt-1 do
						...
						--È¡ ði¬m±»¹¥»÷ÕßÖÜÎ§ÆäËûÍæ¼ÒtoÕ ðµ µ±Ç°×ø±ê	
