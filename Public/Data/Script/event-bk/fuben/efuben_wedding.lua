...
x401030_g_CopySceneType = FUBEN_WEDDING;	-- ¸±±¾ÀàÐÍ,¶¨ÒåTÕi ScriptGlobal.luaÀïÃæ
x401030_g_TickElapse = 5;					-- »Øµ÷½Å±¾toÕ ðµ Ê±ÖÓÊ±¼ä(µ¥Î»:  giây/´Î)
x401030_g_LimitTickCount = 500;				-- ¸±±¾ÐÄÌø´ÎÊýÏÞÖÆ(µ¥Î»: ´ÎÊý)
x401030_g_CloseTick = 6;					-- ¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±ÐÄÌø´ÎÊý(µ¥Î»: ´ÎÊý)
x401030_g_NoUserTime = 300;					-- ¸±±¾ÖÐÃ»ÓÐÈËºó¿ÉÒÔ¼ÌÐø±£´ætoÕ ðµ Ê±¼ä(µ¥Î»:  giây)
x401030_g_Fuben_X = 40;						-- ½ø¸±±¾toÕ ðµ Î»ÖÃX
x401030_g_Fuben_Z = 45;						-- ½ø¸±±¾toÕ ðµ Î»ÖÃZ
x401030_g_Back_X = 175;						-- ³ö¸±±¾toÕ ðµ Î»ÖÃX
x401030_g_Back_Z = 95;						-- ³ö¸±±¾toÕ ðµ Î»ÖÃZ
...
x401030_g_invitationDataId_level2  = 30303101;		-- ¸ßc¤pÇëÌûID
...
function x401030_Create(sceneId, selfId, marryTargetId, weddingLevel)
	...
	LuaFnSetCopySceneData_Param(sceneId, 0, x401030_g_CopySceneType);				--ÉèÖÃ¸±±¾Êý¾Ý,ÕâÀï½«0ºÅË÷ÒýtoÕ ðµ Êý¾ÝÉèÖÃÎª999,ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êý×Ö×Ô¶¨Òå)
	...
	LuaFnSetCopySceneData_Param(sceneId, 6, selfGUID);								--ÉèÖÃÄÐÖ÷ÈËtoÕ ðµ GUID
	LuaFnSetCopySceneData_Param(sceneId, 7, marryTargetGUID);							--ÉèÖÃÅ®Ö÷ÈËtoÕ ðµ GUID
	LuaFnSetCopySceneData_Param(sceneId, 8, weddingLevel);							--»éÇìtoÕ ðµ Àñc¤p±ð
	LuaFnSetCopySceneData_Param(sceneId, CopyScene_LevelGap, 0);					--c¤p±ð²î,CopyScene_LevelGap TÕi  scene.lua ÖÐ¸³Öµ
...
--**********************************************************
--Íæ¼Ò½øÈë¸±±¾
--selfId		:	Òª½øÈë¸±±¾toÕ ðµ Íæ¼ÒID
--targetGUID	:	½øÈëË­toÕ ðµ ¸±±¾(ÄÐ»òÅ®Ö÷½ÇtoÕ ðµ GUID)
--**********************************************************
