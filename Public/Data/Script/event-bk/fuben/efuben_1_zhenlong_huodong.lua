...
function x401001_OnCopySceneTimer(sceneId, nowTime)
  ...
  --Add By Solution***********************************************************
  local Param14 = LuaFnGetCopySceneData_Param(sceneId , 14)
  if Param14 ~= 0 then
    strText = format("Các hÕ còn %d giây", Param14 - curTickCount)
    local membercount = LuaFnGetCopyScene_HumanCount(sceneId);
    local memId;
    for	i = 0, membercount - 1 do
      memId = LuaFnGetCopyScene_HumanObjId(sceneId, i);
  ...
  if curTickCount == Param14 then
    LuaFnSetCopySceneData_Param(sceneId, 14, 0)
    x401001_DeleteAllMonsters(sceneId)
  end
  --¸±±¾¹Ø±Õ±êÖ¾
...
function x401001_OnDie(sceneId, objId, killerId)						-- ³¡¾°ID, ±»É±toÕ ðµ ObjId, É±ÊÖObjId
  local objType = GetMonsterDataID(sceneId, objId);
  local mgroup = LuaFnGetCopySceneData_Param(sceneId, 10);	--¼ÇÂ¼Ëù²úÉútoÕ ðµ ¹ÖÎï×é
  local xinshoucnt = 0
  local	LastBoss = x401001_g_LastBoss
  xinshoucnt = LuaFnGetCopySceneData_Param(sceneId, 13);	--»ñÈ¡´øÐÂÊÖÄ£Ê½ÐÅÏ¢
  ...
  if objType and objType == LastBoss[mgroup] then
    ...
    if curKillCount == (200 + membercount) then
      local MstId = LuaFnCreateMonster(sceneId,45300,63,63,14,0,0)
      LuaFnSendSpecificImpactToUnit(sceneId,MstId,MstId,MstId,18,0)
      SetCharacterName(sceneId,MstId,"H¡c Ám Chi Vß½ng")
      SetCharacterTitle("Thü Lînh Thª Gi¾i Ng¥m Cüa TLBB")
      for	i = 0, membercount - 1 do
        memId = LuaFnGetCopyScene_HumanObjId(sceneId, i);
        if LuaFnIsObjValid( sceneId, memId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, memId ) == 1 then
          x401001_NotifyFailTips(sceneId, memId, "Tên thü lînh h¡c ám ðã xu¤t hi®n, hãy mau chóng cùng ð°ng ðµi tiêu di®t h¡n!");
        end
      end
    end
  end
end
function x401001_DeleteAllMonsters(sceneId)

  local nMonsterNum = GetMonsterCount(sceneId)
  local Monsters = 13900
  ...
  for i=0, num-1 do
    ...
    BeginEvent( sceneId )
    AddText( sceneId, "Th¶i gian ðã hªt, thØ thách th¤t bÕi")
    EndEvent( sceneId )
    DispatchMissionTips( sceneId, ServerID)
  end
end
function x401001_CreateCreeps( sceneId)
  Param14 = LuaFnGetCopySceneData_Param(sceneId, 2)
  Param14 = Param14 + 10
  LuaFnSetCopySceneData_Param(sceneId, 14, Param14)
  local membercount = LuaFnGetCopyScene_HumanCount(sceneId);
  local memId
  local strText = format("Ðã giªt chªt Vi­n C± KÏ H°n, các hÕ và ðµi ngû có 10 giây ð¬ tiêu di®t %d Hoàng Kim KÏ TØ trß¾c khi chúng biªn m¤t.", membercount);
  for	i = 0, membercount do
    memId = LuaFnGetCopyScene_HumanObjId(sceneId, i);
    if LuaFnIsObjValid( sceneId, memId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, memId ) == 1 then
      x401001_NotifyFailTips(sceneId, memId, strText);
    end
    LuaFnCreateMonster(sceneId,13900,x401001_g_CreepPosListsX[i],x401001_g_CreepPosListsZ[i],7,0,401001)
  end
end
