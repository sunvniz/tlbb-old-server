...
function x894065_OnLeaveCombat(sceneId, selfId)
	...
	for i=0, nCount-1  do
		local nObjId = GetMonsterObjID(sceneId, i)
		local MosDataID = GetMonsterDataID( sceneId, nObjId )
		if MosDataID == 15040 or MosDataID == 15045 or MosDataID == 15050 or MosDataID == 15055 or MosDataID == 15060 or MosDataID == 15065
		   MosDataID == 15140 or MosDataID == 15145 or MosDataID == 15150 or MosDataID == 15155 or MosDataID == 15160 or MosDataID == 15165 then
...
function x894065_OnDie( sceneId, selfId, killerId )
	...
	if playerName ~= nil then
		str = format("#cff99cc#{_INFOUSR%s}#cff99cc cßÞng bÑc #GTiêu Nhß Quân#cff99cc không thành, tÑc gi§n hÕ sát y!", playerName); --É£ÍÁ¹«
		AddGlobalCountNews( sceneId, str )
	end

	CallScriptFunction( x894065_g_FuBenScriptId, "TipAllHuman", sceneId, "Tr¥n Dûng Nhân nói:  Tiêu Nhß Quân và Tiêu Nhß Úy tâm tâm tß½ng Ñng!" )
...
function x894065_UseSkillF( sceneId, selfId )
	...
	CallScriptFunction((200060), "Paopao",sceneId, "Tiêu Nhß Quân", "Binh thánh kÏ tr§n", "Tiêu Nhß Quân: ðêm nay bay cùng ta, "..GetName( sceneId, PlayerId ).." hoän loÕn tr¯n chÕy!" )
	CallScriptFunction( x894065_g_FuBenScriptId, "TipAllHuman", sceneId, "Tr¥n Dûng Nhân nói:  Tiêu Nhß Quân mê ho£c lòng ngß¶i" )
...
function x894065_UseSkillG( sceneId, selfId )
	...
	CallScriptFunction( x894065_g_FuBenScriptId, "TipAllHuman", sceneId, "Tr¥n Dûng Nhân nói:  Tiêu Nhß Quân Thi phóng ðích thÑ trúc hãm t¸nh sinh vu túc hÕ, hoàn thïnh chß v¸ t¯c t¯c ðóa t¸." )
...
function x894065_UseSkillH( sceneId, selfId )
		...
		CallScriptFunction( x894065_g_FuBenScriptId, "TipAllHuman", sceneId, "Tr¥n Dûng Nhân nói:  Tiêu Nhß Quân Phát ðµng hßu dßÞng sinh tÑc chi thu§t. Thø ð°ng tâm trúc tß½ng trþ, huynh ð® nh¸ nhân giai dî mi­n d¸ch thß½ng hÕi. Nhßþc nång phá trúc, t¡c ð¯i Ñng phòng ngñ khä b¸ mi­n tr×." )
...
function x894065_OnImpactFadeOut( sceneId, selfId, impactId )
	...
	if impactId == 19413 then
		...
		if bok == 1 then
			...
			CallScriptFunction( x894067_g_FuBenScriptId, "TipAllHuman", sceneId, "Tr¥n Dûng Nhân nói:  30s Tiêu Nhß Quân hi®n dî phøc sinh. . . . . . ." )
