...
--Ñ§Ï°½çÃæÒªËµtoÕ ðµ »°
x713505_g_MessageStudy = "Nªu các hÕ ðÕt t¾i c¤p %d và phäi chi #{_EXCHG%d} m¾i có th¬ h÷c ðßþc kÛ nång chª tÕo. Các hÕ có quyªt ð¸nh h÷c không?"

--¼¼ÄÜ±àºÅ
x713505_g_AbilityID = ABILITY_ZHUZAO

--¼¼ÄÜÃû³Æ
x713505_g_AbilityName = "ðúc khuôn"

x713505_g_Name1 = "Ò®ÂÉ´óÊ¯"
--**********************************
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x713505_OnDefaultEvent( sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId,bid )
	...
	--Íæ¼ÒÖýÔì¼¼ÄÜtoÕ ðµ ÊìÁ·¶È
	ExpPoint = GetAbilityExp(sceneId, selfId, x713505_g_AbilityID)
	--ÈÎÎñÅÐ¶Ï

	--ÅÐ¶ÏÐúng·ñÒÑ¾­Ñ§»áÁËÖýÔì,Èç¹ûÑ§»áÁË,ÔòÌáÊ¾ÒÑ¾­Ñ§»áÁË
	if AbilityLevel >= 1 then
		BeginEvent(sceneId)
        	AddText(sceneId,"Các hÕ ðã h÷c ðßþc"..x713505_g_AbilityName.." kÛ nång");
        	EndEvent(sceneId)
	...
	if ButtomNum == 0 then
		...
		if ret and ret == 1 then
			...
			--Xác nh§nÑ§Ï°°´Å¥
					AddNumText(sceneId,x713505_g_ScriptId,"TÕi hÕ xác ð¸nh mu¯n h÷c", 6, 2)
			--HuÖ böÑ§Ï°°´Å¥
					AddNumText(sceneId,x713505_g_ScriptId,"TÕi hÕ chï mu¯n xem", 8, 3)
			EndEvent(sceneId)
...
function x713505_OnEnumerate( sceneId, selfId, targetId, bid )
		if bid then
			...
			if ret > 0 then AddNumText(sceneId,x713505_g_ScriptId,"H÷c "..x713505_g_AbilityName.." kÛ nång", 12, 0) end
			return
		end
		...
		if ret and ret == 1 then
			AddNumText(sceneId,x713505_g_ScriptId,"H÷c "..x713505_g_AbilityName.." kÛ nång", 12, 0)
		end
		return
end
...
function x713505_StudyInCity(sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId,bid)
	if bid then
		if 0 == ButtomNum then
			...
			local studyMsg = format("Nªu các hÕ ðÕt t¾i c¤p %d, phäi chi #{_EXCHG%d }và %d ði¬m bang hµi s¨ có th¬ h÷c ðßþc"..x713505_g_AbilityName.."KÛ nång. Ngß½i quyªt ð¸nh h÷c cái gì?", lv, money, con)
