...
function x713565_OnDefaultEvent( sceneId, selfId, targetId, nNum, npcScriptId, bid )
	...
	--Èç¹ûĞúngTÕi Thành ph¯ ÖĞThång c¤p 
	if bid then
	...
	--Èç¹ûÉú»î kÛ nångµÈc¤pÒÑ¾­³¬³ö¸ÃnpcËùÄÜ½ÌtoÕ ğµ ·¶Î§
	if AbilityLevel >= MaxLevel then
		BeginEvent(sceneId)
			if GetName(sceneId, targetId) == x713565_g_Name1   then
				strText = "Ta chï có th¬ dÕy ngß½i t× c¤p 1-10"..x713565_g_AbilityName.." kÛ nång"
			else
			strText = "Ta chï có th¬ dÕy ngß½i t× c¤p 1-5"..x713565_g_AbilityName.." kÛ nång, xin t¾i bang hµi ğ¬ h÷c cao h½n! #YMµc Uy¬n Thanh #G(ĞÕi Lı#{_INFOAIM107,135,2,Mµc Uy¬n Thanh}) #Wtìm hi¬u thêm"..x713565_g_AbilityName..""
			end
			AddText(sceneId,strText)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
