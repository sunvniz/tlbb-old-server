...
--Ñ§Ï°½çÃæÒªËµtoÕ ðµ »°
x713533_g_MessageStudy = "Chï c¥n các hÕ ch¸u bö #{_MONEY%d} m¾i có th¬ h÷c ðßþc kÛ nång kinh mÕch bách quyªt. Các hÕ có quyªt ð¸nh h÷c không?"
...
function x713533_OnDefaultEvent( sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId )
	...
	--ÅÐ¶ÏÐúng·ñÒÑ¾­Ñ§»áÁË¾­Âö°Ù¾ö,Èç¹ûÑ§»áÁË,ÔòÌáÊ¾ÒÑ¾­Ñ§»áÁË
	if AbilityLevel >= 1 then
		BeginEvent(sceneId)
        	AddText(sceneId,"Các hÕ ðã h÷c kÛ nång kinh mÕch bách quyªt");
        	EndEvent(sceneId)
	...
	if ButtomNum == 0 then
		...
		if ret and ret == 1 then
			...
			--Xác nh§nÑ§Ï°°´Å¥
					AddNumText(sceneId,x713533_g_ScriptId,"TÕi hÕ xác ð¸nh mu¯n h÷c", 6, 2)
			--HuÖ böÑ§Ï°°´Å¥
					AddNumText(sceneId,x713533_g_ScriptId,"TÕi hÕ chï mu¯n coi", 8, 3)
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
...
function x713533_OnEnumerate( sceneId, selfId, targetId )
		...
		--if GetLevel(sceneId,selfId) >= 10 then
			AddNumText(sceneId,x713533_g_ScriptId,"H÷c kÛ nång kinh mÕch bách quyªt", 12, 0)
		--end
		return
end
