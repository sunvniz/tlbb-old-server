...
--Ñ§Ï°½çÃæÒªËµtoÕ ðµ »°
x713514_g_MessageStudy = "Nªu các hÕ ðÕt t¾i c¤p %d và phäi tiêu t¯n #{_MONEY%d} m¾i có th¬ h÷c ðßþc kÛ nång làm rßþu. Các hÕ có quyªt ð¸nh h÷c không?"
...
function x713514_OnDefaultEvent( sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId )
	...
	--ÅÐ¶ÏÐúng·ñÒÑ¾­Ñ§»áÁËÄð¾Æ,Èç¹ûÑ§»áÁË,ÔòÌáÊ¾ÒÑ¾­Ñ§»áÁË
	if AbilityLevel >= 1 then
		BeginEvent(sceneId)
        	AddText(sceneId,"Các hÕ ðã h÷c xong kÛ nång làm rßþu");
        	EndEvent(sceneId)
	...
	if ButtomNum == 0 then
		...
		if ret and ret == 1 then
			...
			--Xác nh§nÑ§Ï°°´Å¥
					AddNumText(sceneId,x713514_g_ScriptId,"TÕi hÕ xác ð¸nh mu¯n h÷c", 6, 2)
			--HuÖ böÑ§Ï°°´Å¥
					AddNumText(sceneId,x713514_g_ScriptId,"TÕi hÕ chï mu¯n coi", 8, 3)
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
...
function x713514_OnEnumerate( sceneId, selfId, targetId )
		...
		--if GetLevel(sceneId,selfId) >= 10 then
			AddNumText(sceneId,x713514_g_ScriptId,"H÷c kÛ nång làm rßþu", 12, 0)
		--end
		return
end
