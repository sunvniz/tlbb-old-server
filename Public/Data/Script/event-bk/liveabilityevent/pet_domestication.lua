...
function x701603_OnDefaultEvent( sceneId, selfId, targetId, ButtomNum )
	--¼ÆËãÕäÊŞÊıÁ¿,Èç¹ûĞúng0,Ôò¸ø³öÏàÓ¦ÌáÊ¾
	...
	--»ñÈ¡³öÕ½ÕäÊŞtoÕ ğµ ObjId
	local	ObjId			= x701603_FightingPet( sceneId, selfId )
	
	--¼ÆËã¸øÕäÊŞTr¸ li®uC¥n ¶àÉÙÇ®
	local	MoneyCost	= 0
	local i
	...
	if ButtomNum == PET_FULL then
		BeginEvent( sceneId )
			AddText( sceneId, "  Các hÕ c¥n chi #G#{_EXCHG"..MoneyCost.."}#W, xin xác ğ¸nh có mu¯n trân thú h°i phøc không?" )
			--Xác nh§n¸øÕäÊŞ¼ÓÑª°´Å¥
	...
	elseif ButtomNum == PET_FULL_OK then
		...
		if (nMoneyJZ + nMoney >= MoneyCost) then
			...
			--TÕi npcÁÄÌì´°¿ÚÍ¨ÖªÍæ¼mµt ¨·ÑÁË¶àÉÙÇ®,²¢¸æËßÍæ¼ÒÕäÊŞÒÑ¾­»Ö¸´ÁË
			...
			elseif (moneyJZ == 0) and ( money == MoneyCost) then
				...
				BeginEvent( sceneId )
					AddText( sceneId, "  Chæa tr¸ không thành công!" )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
			end
...
function x701603_OnEnumerate( sceneId, selfId, targetId )
	--Ìí¼Ó°´Å¥
	AddNumText(sceneId,x701603_g_ScriptId,"Tr¸ thß½ng cho trân thú", 6,PET_FULL)	--ÕâÀïtoÕ ğµ PET_FULLĞúngÓÃÀ´±íÊ¾Cái này °´Å¥ĞúngµÚmµt cáiÒ³ÃætoÕ ğµ ÏÔÊ¾°´Å¥,²»ĞúngXác nh§n»Ö¸´toÕ ğµ °´Å¥
	return
end
...
--¼ÆËã»Ö¸´Ñª·ÑÓÃ
--**********************************
--µ¥Î»ÑªÑ±Ñø¼ÛÖµ: 0.025+n*0.0005(nÎªÕäÊŞµÈc¤p)
--µ¥Î»¿ìÀÖ¶È¼ÛÖµ: 0.373+0.44*n(nÎªÕäÊŞµÈc¤p)
