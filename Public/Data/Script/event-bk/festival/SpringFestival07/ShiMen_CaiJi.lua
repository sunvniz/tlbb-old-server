...
--¶mµt »Ê±¼ä
x050059_ChangeTime_Start =  09024 --¶mµt »¿ªÊ¼2009Äê1ÔÂ25ÈÕ0Ê±
x050059_ChangeTime_End   =  09037  --¶mµt »½ØÖÁ2009Äê2ÔÂ7ÈÕ24Ê±
...
function x050059_GetGift(sceneId, selfId, targetId,MenPai)
	...
	if LuaFnGetPropertyBagSpace( sceneId, selfId ) < 1 then
		...
		x050059_CloseWindow(sceneId, selfId,targetId)
		-- ðÕt ðßþc¸÷cáic¤p±ðÕäÊÞtoÕ ðµ Íæ¼Ò
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x050059_OnDefaultEvent( sceneId, selfId, targetId,MenPai,MissionID,ScriptID  )	-- ði¬m»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	--¼ì²âÃÅÅÉÐÅÏ¢Ðúng·ñºÏ·¨
...
--**********************************
--  ðÕt ðßþc½ñÌìtoÕ ðµ ÈÎÎñÊýÁ¿ -1±íÊ¾½ñÌìtoÕ ðµ Nhi®m vø quá hÕn¶î
--**********************************
function x050059_GetTaskCount(sceneId,selfId)
	--ÅÐ¶Ï½ñÌìtoÕ ðµ ÈÎÎñÁ¿Ðúng·ñ³¬³ö
		local iTaskCount =0
		local td = GetDayTime()
		...
		if td < LastGetTaskTime then
			iTaskCount = -1
		--µ±Ç°ÈÕÆÚ,¼ì²âµ±ÌìtoÕ ðµ S¯ l¥n nhi®m vø 
		else
			local TaskCount = GetMissionData(sceneId,selfId,MD_SHIMENCAIJI_COUNT)
			--µ±ÌìtoÕ ðµ S¯ l¥n nhi®m vø ´óÓÚ24,»¹ÓÐ¿ÉÄÜÐúng½»ÈÎÎñ,ËùÒÔ24TÕi ´Îº¯ÊýÖÐÈÏÎªÐúngºÏ·¨´ÎÊý
...
--**********************************
-- Thöa mãn»ñÈ¡ÈÎÎñtoÕ ðµ Ìõ¼þ,Tiªp thøÈÎÎñ
--**********************************
function x050059_AcceptTask(sceneId, selfId,targetId,MenPai,MissionID,ScriptID)
	...
	--½ñÌìtoÕ ðµ S¯ l¥n nhi®m vø ÒÑÂú,ÐÑÄ¿ÌáÊ¾,ÍË³ö
	...
	--24´ÎtoÕ ðµ Ê±ºòÖ»ÄÜ½»ÈÎÎñ,²»ÄÜ½ÓÈÎÎñ
...
function x050059_Accept(sceneId, selfId,MissionID,ScriptID)
	...
	Msg2Player(sceneId, selfId, "#YNh§n nhi®m vø: ÃÅÅÉÄê»õ´ó²É¹º", MSG2PLAYER_PARA )
...
function x050059_OnSubmit( sceneId, selfId, targetId, selectRadioId,MissionID )
	...
	if x050059_CheckSubmit( sceneId, selfId,MissionID)>=1 then
		...
		--µÚ8»·Òª½±ÀøÎïÆ·,¼ì²âÐúng·ñÓÐ±³°ü¿Õ¼ä
		if (iDayHuan==0) then
			-- ¼ì²é±³°ü¿Õ¼ä
			if LuaFnGetPropertyBagSpace( sceneId, selfId ) < 1 then
					x050059_MsgBox( sceneId, selfId, "#{LLFB_80821_6}")
...
function x050059_OnSubmit_Necessary( sceneId, selfId, targetId)
	...
	--ÉèÖÃµ±ÌìtoÕ ðµ ÈÎÎñ×ÜÊý(Tiªp thøÈÎÎñtoÕ ðµ Ê±ºòÒÑ¾­¼ÇÂ¼ÁË´ÎÊý,ËùÒÔ×¢ÊÍÁË´ËÐÐ
...
function x050059_OnAbandon( sceneId, selfId,MissionID)
	...
	if IsHaveMission(sceneId, selfId, MissionID) > 0 then	
	  ...
	  DelMission( sceneId, selfId, MissionID )
		local td = GetDayTime()
		local LastGetTaskTime = GetMissionData(sceneId,selfId,MD_SHIMENCAIJI_DAY)
		--µ±Ç°ÈÕÆÚÐúng·ñ´óÓÚÒÑ¾­¼ÇÂ¼toÕ ðµ ÈÕÆÚ
		if td > LastGetTaskTime then
			SetMissionData(sceneId, selfId, MD_SHIMENCAIJI_DAY, td)
