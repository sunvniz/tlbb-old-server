...
x890063_g_NoUserTime	= 10			--¸±±¾ÖÐÃ»ÓÐ  nhân ºó¿ÉÒÔ¼ÌÐø±£´æµÄÊ±¼ä£¨µ¥Î»£ºÃë£©
x890063_g_Fuben_X			= 128			--½ø  nhân ¸±±¾µÄÎ»ÖÃX
x890063_g_Fuben_Z			= 225			--½ø  nhân ¸±±¾µÄÎ»ÖÃZ
...
x890063_g_BOSSList =
{
	...
	["DingChunQiu_BOSS"]		= { DataID=42207, Title="Thi¬u th¤t s½n th¤t chü", posX=129, posY=127, Dir=27, BaseAI=27, AIScript=0, ScriptID=890069 },
}
...
--ÊÇ·ñ¿ÉÒÔÌôÕ½Ä³ cái BOSSµÄ±ê¼ÇÁÐ±í....
x890063_g_BattleFlagTbl =
{
	["JiuMoZhi"]			= 8,	--ÊÇ·ñ¿ÉÒÔÌôÕ½¹þTheo °Ô...
	["ZhuangJuXian"]	= 9,	--ÊÇ·ñ¿ÉÒÔÌôÕ½É£ÍÁ¹«....
	["MuRongFu"]			= 10,	--ÊÇ·ñ¿ÉÒÔÌôÕ½ÎÚÀÏTheo ....
	["ShuangZi"]		= 11,	--ÊÇ·ñ¿ÉÒÔÌôÕ½Ë«×Ó....
	["DingChunQiu"]		= 12,	--ÊÇ·ñ¿ÉÒÔÌôÕ½ÀîÇïË®....
}

--³¡¾°±äÁ¿Ë÷Òý....ÊÇ·ñ¿ÉÒÔÌôÕ½Ä³ cái BOSSµÄ±ê¼Ç....
...
--³¡¾°±äÁ¿Ë÷Òý....ÎÚÀÏTheo  ðßþc ÍöµÄ¼ÆÊ±Æ÷....ÓÃÓÚ´¦Àí ðßþc ÍöÂß¼­....
...
function x890063_OnEnumerate( sceneId, selfId, targetId )
	...
	--AddNumText( sceneId, x890063_g_ScriptId, "Thiªu Th¤t S½n", 10, 1 )

end

--**********************************
--¼ì²âÊÇ·ñ¿ÉÒÔ½ø  nhân ´Ë¸±±¾....
--**********************************
function x890063_CheckCanEnter( sceneId, selfId, targetId )
	...
	if nHumanNum > 0 then
		...
		msg = msg .. Humanlist[nHumanNum-1] .. " tu vi còn th¤p, không th¬ ði"
		return 0, msg

	end


	--ÊÇ·ñÓÐ  nhân ½ñÌì×ö¹ý3´ÎÁË....
	nHumanNum = 0
	...
	if nHumanNum > 0 then
		...
		msg = msg .. Humanlist[nHumanNum-1] .. " hôm nay ðã khiêu chiªn 3 l¥n Thiªu Th¤t S½n"
		return 0, msg
...
function x890063_MakeCopyScene( sceneId, selfId )
	...
	BeginEvent(sceneId)
		if bRetSceneID>0 then
			...
			AddText(sceneId,"S¯ lßþng bän sao ðã ðÕt gi¾i hÕn, hãy ðþi bäo trì máy chü!");
		end
	EndEvent(sceneId)
...
function x890063_OnCopySceneReady( sceneId, destsceneId )
	...
	--½ø  nhân ¸±±¾µÄ¹æÔò
	-- 1,Èç¹ûÕâ cái Íæ¼ÒÃ»ÓÐ×é¶Ó,¾Í´«ËÍÕâ cái Íæ¼Ò×Ô¼º½ø  nhân ¸±±¾
	-- 2, Èç¹ûÍæ¼ÒÓÐ¶ÓÎé,µ«ÊÇÍæ¼Ò²»ÊÇ¶Ó³¤,¾Í´«ËÍ×Ô¼º½ø  nhân ¸±±¾
	-- 3,Èç¹ûÍæ¼ÒÓÐ¶ÓÎé,²¢ÇÒÕâ cái Íæ¼ÒÊÇ¶Ó³¤,¾Í´«ËÍ×Ô¼ººÍ¸½½ü¶ÓÓÑÒ»Æð½øÈ¥

	LuaFnSetCopySceneData_Param(destsceneId, 3, sceneId) --ÉèÖÃ¸±±¾  nhân ¿Ú³¡¾°ºÅ
...
--**********************************
--Tick½£ÎèÇøÓò....
--Ö»ÒªÍæ¼ÒÕ¾ÔÚ³¡¾°ÀïµÄ6 cái ¹âÖùÄÚ....Ã¿Ãë¶¼ÄÜ»ñµÃÒ» cái ÃâÒß½£ÎèµÄbuff....
--**********************************
...
function x890063_CheckHaveBOSS( sceneId )
	...
	if nBossNum > 0 then
		...
		msg = msg .. BossList[nBossNum-1] .. " Ðang chiªn ð¤u "
		return 1, msg
	end
...
--**********************************
--»ñÈ¡ÊÇ·ñ¿ÉÒÔÌôÕ½Ä³ cái BOSSµÄ±ê¼Ç....
--**********************************
...
--**********************************
--ÉèÖÃÊÇ·ñ¿ÉÒÔÌôÕ½Ä³ cái BOSSµÄ±ê¼Ç....
--**********************************
